...
proof -
	...
	have "col F H H" using collinear_b `axioms` `H = H` by blast
	...
	have "H = H" using equalityreflexiveE[OF `axioms`] .
	have "col H K H" using collinear_b `axioms` `H = H` by blast
	...
	have "bet E H F" using `bet E H F` .
	have "col C K D" using collinear_b `axioms` `bet C K D` by blast
	have "col E H F" using collinear_b `axioms` `bet E H F` by blast
	...
	have "col F N C" using collinear_b `axioms` `bet F N C \<and> col H K N \<and> \<not> col H K F` by blast
	have "N = N" using equalityreflexiveE[OF `axioms`] .
	have "col F N N" using collinear_b `axioms` `N = N` by blast
	...
	have "col H N K" using collinear_b `axioms` `bet H N K` by blast
	have "col H N H" using collinear_b `axioms` `H = H` by blast
	...
	have "same_side E C H K" using sameside_b[OF `axioms` `col H K H` `col H K N` `bet E H F` `bet C N F` `\<not> col H K E` `\<not> col H K C`] .
	have "K = K" using equalityreflexiveE[OF `axioms`] .
	have "col H K K" using collinear_b `axioms` `K = K` by blast
	have "oppo_side C H K D" using oppositeside_b[OF `axioms` `bet C K D` `col H K K` `\<not> col H K C`] .
	...
	have "col G H K" using collinear_b `axioms` `bet G H K` by blast
	have "G \<noteq> H" using betweennotequal[OF `axioms` `bet G H K`] by blast
	...
	have "same_side A C G K" using sameside_b[OF `axioms` `col G K M` `col G K N` `bet A M F` `bet C N F` `\<not> col G K A` `\<not> col G K C`] .
	have "col G K K" using collinear_b `axioms` `K = K` by blast
	have "oppo_side C G K D" using oppositeside_b[OF `axioms` `bet C K D` `col G K K` `\<not> col G K C`] .
