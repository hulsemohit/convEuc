...
proof -
	...
	have "col E C C" using col_b `axioms` `C = C` by blast
	...
	have "\<not> col b e a" using NChelper[OF `axioms` `\<not> col b c a` `col b c b` `col b c e` `b \<noteq> e`] .
	have "\<not> col a e b" using NCorder[OF `axioms` `\<not> col b e a`] by blast
	have "triangle A E B" sorry
	have "tri_cong A E B a e b" sorry
	...
	have "C = C" using equalityreflexiveE[OF `axioms`] .
	have "col C B C" using col_b `axioms` `C = C` by blast
	...
	have "triangle A E C" sorry
	have "tri_cong A E C a e c" sorry
	...
	have "col B B C" using col_b `axioms` `B = B` by blast
