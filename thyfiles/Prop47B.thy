...
proof -
	...
	have "parallelogram A B F G" using squareparallelogram[OF `axioms` `square A B F G`] .
	have "parallel A B F G" using parallelogram_f[OF `axioms` `parallelogram A B F G`] by blast
	have "parallel A B G F" using parallelflip[OF `axioms` `parallel A B F G`] by blast
	have "tarski_parallel A B G F" using paralleldef2B[OF `axioms` `parallel A B G F`] .
	have "same_side G F A B" using tarski_parallel_f[OF `axioms` `tarski_parallel A B G F`] by blast
	...
	have "parallel A G B F" using parallelogram_f[OF `axioms` `parallelogram A B F G`] by blast
	have "parallel A G F B" using parallelflip[OF `axioms` `parallel A G B F`] by blast
	have "col G A C" using collinear_b `axioms` `bet G A C` by blast
	have "col A G C" using collinearorder[OF `axioms` `col G A C`] by blast
	have "G \<noteq> C" using betweennotequal[OF `axioms` `bet G A C`] by blast
	...
	have "F \<noteq> B" using NCdistinct[OF `axioms` `\<not> col A B F`] by blast
	have "B = B" using equalityreflexiveE[OF `axioms`] .
	have "col F B B" using collinear_b `axioms` `B = B` by blast
	...
	have "bet B a A" using collinearbetween[OF `axioms` `col F B B` `col G A C` `F \<noteq> B` `G \<noteq> C` `F \<noteq> B` `A \<noteq> C` `\<not> (meets F B G C)` `bet F a C` `col B A a`] .
	have "B \<noteq> a" using betweennotequal[OF `axioms` `bet B a A`] by blast
	have "ray_on B a A" using ray4 `axioms` `bet B a A` `B \<noteq> a` by blast
	...
	have "\<not> col A B F" using parallelNC[OF `axioms` `parallel A B F G`] by blast
	have "\<not> col F B A" using NCorder[OF `axioms` `\<not> col A B F`] by blast
	have "ang_eq F B A F B A" using equalanglesreflexive[OF `axioms` `\<not> col F B A`] .
	have "ray_on B A a" using ray5[OF `axioms` `ray_on B a A`] .
	have "ang_eq F B A F B a" using equalangleshelper[OF `axioms` `ang_eq F B A F B A` `ray_on B F F` `ray_on B A a`] .
	...
	have "parallel C E D B" using parallelflip[OF `axioms` `parallel C E B D`] by blast
	have "col B C c" using collinearorder[OF `axioms` `col C B c`] by blast
	have "col B M C" using collinear_b `axioms` `parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A` by blast
	have "col C B M" using collinearorder[OF `axioms` `col B M C`] by blast
	have "col C B c" using collinearorder[OF `axioms` `col B C c`] by blast
	have "C \<noteq> B" using NCdistinct[OF `axioms` `\<not> col A B C`] by blast
	have "col B M c" using collinear4[OF `axioms` `col C B M` `col C B c` `C \<noteq> B`] .
	have "parallelogram B M L D" using `parallelogram B M L D` .
	have "parallel B D M L" using parallelogram_f[OF `axioms` `parallelogram B M L D`] by blast
	have "col L M A" using collinear_b `axioms` `parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A` by blast
	have "col M L A" using collinearorder[OF `axioms` `col L M A`] by blast
	have "L \<noteq> A" using betweennotequal[OF `axioms` `bet L M A`] by blast
	have "A \<noteq> L" using inequalitysymmetric[OF `axioms` `L \<noteq> A`] .
	...
	have "bet B c M" using collinearbetween[OF `axioms` `col D B B` `col L M A` `D \<noteq> B` `L \<noteq> A` `D \<noteq> B` `M \<noteq> A` `\<not> (meets D B L A)` `bet D c A` `col B M c`] .
	have "bet B c C" using n3_6b[OF `axioms` `bet B c M` `bet B M C`] .
	...
	proof (rule ccontr)
		...
		have "D \<noteq> c" using betweennotequal[OF `axioms` `bet D c A`] by blast
		have "c \<noteq> D" using inequalitysymmetric[OF `axioms` `D \<noteq> c`] .
		have "col D B A" using collinear4[OF `axioms` `col c D B` `col c D A` `c \<noteq> D`] .
		show "False" using `col D B A` `\<not> col D B A` by blast
	qed
	hence "B \<noteq> c" by blast
	have "ray_on B c C" using ray4 `axioms` `bet B c C` `B \<noteq> c` by blast
	have "ray_on B C c" using ray5[OF `axioms` `ray_on B c C`] .
	...
	have "ang_eq C B A c B A" using equalangleshelper[OF `axioms` `ang_eq C B A C B A` `ray_on B C c` `ray_on B A A`] .
	have "\<not> col C D B" using parallelNC[OF `axioms` `parallel C E D B`] by blast
	have "\<not> col D B C" using NCorder[OF `axioms` `\<not> col C B D`] by blast
	have "ang_eq D B C D B C" using equalanglesreflexive[OF `axioms` `\<not> col D B C`] .
	have "B \<noteq> D" using inequalitysymmetric[OF `axioms` `D \<noteq> B`] .
	have "ray_on B D D" using ray4 `axioms` `D = D` `B \<noteq> D` by blast
	have "ang_eq D B C D B c" using equalangleshelper[OF `axioms` `ang_eq D B C D B C` `ray_on B D D` `ray_on B C c`] .
	have "area_sum_eq D B C C B A D B A" using anglesum_b[OF `axioms` `ang_eq D B C D B c` `ang_eq C B A c B A` `bet D c A`] .
	...
	have "ang_eq F B A D B C" using Euclid4[OF `axioms` `ang_right F B A` `ang_right D B C`] .
	have "ang_eq A B C C B A" using ABCequalsCBA[OF `axioms` `\<not> col A B C`] .
	have "ang_eq F B C D B A" using angleaddition[OF `axioms` `area_sum_eq F B A A B C F B C` `ang_eq F B A D B C` `ang_eq A B C C B A` `area_sum_eq D B C C B A D B A`] .
	...
	have "\<not> col F B C" using NCorder[OF `axioms` `\<not> col C B F`] by blast
	have "ang_eq F B C C B F" using ABCequalsCBA[OF `axioms` `\<not> col F B C`] .
	have "ang_eq D B A C B F" using equalanglestransitive[OF `axioms` `ang_eq D B A F B C` `ang_eq F B C C B F`] .
	have "seg_eq A B B F" using square_f[OF `axioms` `square A B F G`] by blast
	have "seg_eq A B F B" using congruenceflip[OF `axioms` `seg_eq A B B F`] by blast
	have "seg_eq F B A B" using congruencesymmetric[OF `axioms` `seg_eq A B F B`] .
	have "seg_eq B F B A" using congruenceflip[OF `axioms` `seg_eq F B A B`] by blast
	have "seg_eq B A B F" using congruencesymmetric[OF `axioms` `seg_eq B F B A`] .
	have "seg_eq B C D B" using square_f[OF `axioms` `square B C E D`] by blast
	have "seg_eq D B B C" using congruencesymmetric[OF `axioms` `seg_eq B C D B`] .
	have "seg_eq B D B C" using congruenceflip[OF `axioms` `seg_eq D B B C`] by blast
	have "seg_eq D A C F \<and> ang_eq B D A B C F \<and> ang_eq B A D B F C" using Prop04[OF `axioms` `seg_eq B D B C` `seg_eq B A B F` `ang_eq D B A C B F`] .
	have "seg_eq D A C F" using `seg_eq D A C F \<and> ang_eq B D A B C F \<and> ang_eq B A D B F C` by blast
	have "seg_eq A D F C" using congruenceflip[OF `axioms` `seg_eq D A C F`] by blast
	have "ang_eq B A D B F C" using `seg_eq D A C F \<and> ang_eq B D A B C F \<and> ang_eq B A D B F C` by blast
	have "ang_eq B F C B A D" using equalanglessymmetric[OF `axioms` `ang_eq B A D B F C`] .
	have "\<not> col B A D" using equalanglesNC[OF `axioms` `ang_eq B F C B A D`] .
	have "\<not> col A B D" using NCorder[OF `axioms` `\<not> col B A D`] by blast
	...
	have "tri_cong A B D F B C" using trianglecongruence_b[OF `axioms` `seg_eq A B F B` `seg_eq B D B C` `seg_eq A D F C` `triangle A B D`] .
	have "tri_eq_area A B D F B C" using congruentequalE[OF `axioms` `tri_cong A B D F B C`] .
	have "parallelogram B M L D" using `parallelogram B M L D` .
	...
	have "parallel M B D L" using parallelflip[OF `axioms` `parallel B M L D`] by blast
	have "parallelogram M B D L" using parallelogram_b[OF `axioms` `parallel M B D L` `parallel M L B D`] .
	have "col M L A" using collinearorder[OF `axioms` `col L M A`] by blast
	have "tri_eq_area M B D A B D" using Prop41[OF `axioms` `parallelogram M B D L` `col M L A`] .
	have "parallelogram A B F G" using squareparallelogram[OF `axioms` `square A B F G`] .
	have "parallelogram B A G F" using PGflip[OF `axioms` `parallelogram A B F G`] .
	have "col G A C" using collinear_b `axioms` `bet G A C` by blast
	have "col A G C" using collinearorder[OF `axioms` `col G A C`] by blast
	have "tri_eq_area A B F C B F" using Prop41[OF `axioms` `parallelogram A B F G` `col A G C`] .
	have "tri_eq_area A B F F B C" using ETpermutationE[OF `axioms` `tri_eq_area A B F C B F`] by blast
	have "tri_eq_area F B C A B D" using ETsymmetricE[OF `axioms` `tri_eq_area A B D F B C`] .
	have "tri_eq_area A B F A B D" using ETtransitiveE[OF `axioms` `tri_eq_area A B F F B C` `tri_eq_area F B C A B D`] .
	have "tri_eq_area A B D M B D" using ETsymmetricE[OF `axioms` `tri_eq_area M B D A B D`] .
	have "tri_eq_area A B F M B D" using ETtransitiveE[OF `axioms` `tri_eq_area A B F A B D` `tri_eq_area A B D M B D`] .
	have "tri_cong A B F F G A" using Prop34[OF `axioms` `parallelogram B A G F`] by blast
	have "tri_eq_area A B F F G A" using congruentequalE[OF `axioms` `tri_cong A B F F G A`] .
	have "parallelogram B M L D" using PGflip[OF `axioms` `parallelogram M B D L`] .
	have "tri_cong M B D D L M" using Prop34[OF `axioms` `parallelogram B M L D`] by blast
	have "tri_eq_area M B D D L M" using congruentequalE[OF `axioms` `tri_cong M B D D L M`] .
	have "tri_eq_area F G A A B F" using ETsymmetricE[OF `axioms` `tri_eq_area A B F F G A`] .
	have "tri_eq_area F G A A B D" using ETtransitiveE[OF `axioms` `tri_eq_area F G A A B F` `tri_eq_area A B F A B D`] .
	have "tri_eq_area F G A M B D" using ETtransitiveE[OF `axioms` `tri_eq_area F G A A B D` `tri_eq_area A B D M B D`] .
	have "tri_eq_area F G A D L M" using ETtransitiveE[OF `axioms` `tri_eq_area F G A M B D` `tri_eq_area M B D D L M`] .
	have "tri_eq_area F G A D M L" using ETpermutationE[OF `axioms` `tri_eq_area F G A D L M`] by blast
	have "tri_eq_area D M L F G A" using ETsymmetricE[OF `axioms` `tri_eq_area F G A D M L`] .
	have "tri_eq_area D M L F A G" using ETpermutationE[OF `axioms` `tri_eq_area D M L F G A`] by blast
	have "tri_eq_area F A G D M L" using ETsymmetricE[OF `axioms` `tri_eq_area D M L F A G`] .
	have "tri_eq_area A B F D M B" using ETpermutationE[OF `axioms` `tri_eq_area A B F M B D`] by blast
	...
	have "bet M n D" using midpoint_f[OF `axioms` `midpoint M n D`] by blast
	have "bet D n M" using betweennesssymmetryE[OF `axioms` `bet M n D`] .
	have "col M n D" using collinear_b `axioms` `bet M n D` by blast
	...
	have "qua_eq_area A B F G B M L D" using EFsymmetricE[OF `axioms` `qua_eq_area B M L D A B F G`] .
