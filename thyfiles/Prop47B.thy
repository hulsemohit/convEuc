...
proof -
	...
	obtain a where "bet F a C \<and> col A B a \<and> \<not> col A B F" sorry
	...
	have "parallel A G B F" sorry
	have "parallel A G F B" using parallelflip[OF `axioms` `parallel A G B F`] by blast
	have "col G A C" using col_b `axioms` `bet G A C` by blast
	...
	have "col F B B" using col_b `axioms` `B = B` by blast
	...
	have "area_sum_eq F B A A B C F B C" sorry
	have "oppo_side D C B A" using `oppo_side D C B A` .
	obtain c where "bet D c A \<and> col C B c \<and> \<not> col C B D" sorry
	...
	have "col B M C" using col_b `axioms` `parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A` by blast
	...
	have "parallel B D M L" sorry
	have "col L M A" using col_b `axioms` `parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A` by blast
	...
	have "col D B B" using col_b `axioms` `B = B` by blast
	...
	proof (rule ccontr)
		...
		have "col D c A" using col_b `axioms` `bet D c A \<and> col C B c \<and> \<not> col C B D` by blast
	...
	have "area_sum_eq D B C C B A D B A" sorry
	...
	have "tri_cong A B D F B C" sorry
	...
	have "parallelogram M B D L" sorry
	have "col M L A" using collinearorder[OF `axioms` `col L M A`] by blast
	...
	have "bet A m F" sorry
	have "bet B m G" sorry
	...
	have "bet B n L" sorry
	have "bet M n D" sorry
	have "bet D n M" using betweennesssymmetryE[OF `axioms` `bet M n D`] .
	have "col M n D" using col_b `axioms` `bet M n D` by blast
