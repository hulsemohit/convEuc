...
proof -
	...
	have "col A M Q" using col_b `axioms` `bet B Q F \<and> bet A M Q` by blast
	...
	have "col F A A" using col_b `axioms` `A = A` by blast
	...
	proof (rule ccontr)
		...
		have "col E H B" using col_b `axioms` `bet E H B \<and> bet F r H` by blast
		have "col F r H" using col_b `axioms` `bet E H B \<and> bet F r H` by blast
		have "col E B H" using collinearorder[OF `axioms` `col E H B`] by blast
		have "col C r F" using col_b `axioms` `bet C r F \<and> bet E r Q` by blast
	...
	have "A = F \<or> A = E \<or> F = E \<or> bet F A E \<or> bet A F E \<or> bet A E F" using col_f[OF `axioms` `col A F E`] .
	...
	proof (cases)
		...
		proof (rule ccontr)
			...
			have "bet A D F" using `bet A D F` .
			have "bet A D A" sorry
			have "\<not> (bet A D A)" using betweennessidentityE[OF `axioms`] .
	...
	next
		...
		proof (rule ccontr)
			...
			have "col B E F" using col_b `axioms` `E = F` by blast
			have "col E B F" using collinearorder[OF `axioms` `col B E F`] by blast
			have "F = F" using equalityreflexiveE[OF `axioms`] .
			have "col F C F" using col_b `axioms` `F = F` by blast
			have "E \<noteq> B" sorry
			have "F \<noteq> C" sorry
	...
	next
		...
		proof (rule ccontr)
			...
			have "seg_eq F A F A" using congruencereflexiveE[OF `axioms`] .
			have "seg_lt F A F E" sorry
