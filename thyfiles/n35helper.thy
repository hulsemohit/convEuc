...
proof -
	...
	obtain Q where "bet B Q F \<and> bet A M Q" using Pasch_outerE[OF `axioms` `bet B M D` `bet A D F` `\<not> col A F B`]  by  blast
...
hence "bet A F E" by blast
		...
		obtain r where "bet C r F \<and> bet E r Q" using Pasch_innerE[OF `axioms` `bet C Q A` `bet E F A` `\<not> col C A E`]  by  blast
		...
		obtain H where "bet E H B \<and> bet F r H" using Pasch_outerE[OF `axioms` `bet E r Q` `bet F Q B` `\<not> col F B E`]  by  blast
		...
		have "F \<noteq> C" using parallel_f[OF `axioms` `parallel E B F C`] by fastforce
		...
		have "\<not> (meets E B F C)" using parallel_f[OF `axioms` `parallel E B F C`] by fastforce
...
hence "\<not> (bet A E F)" by blast
			...
			have "E \<noteq> B" using parallel_f[OF `axioms` `parallel E B C F`] by fastforce
			have "F \<noteq> C" using parallel_f[OF `axioms` `parallel E B F C`] by fastforce
