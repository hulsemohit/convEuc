...
proof -
	...
	proof (rule ccontr)
		...
		have "\<not> (bet C G C)" using betweennessidentityE[OF `axioms`] by blast
	...
	have "seg_eq Q C C Q" using equalityreverseE[OF `axioms`] by blast
	have "seg_eq Q C E P" using congruencetransitive[OF `axioms` `seg_eq Q C C Q` `seg_eq C Q E P`] .
	have "seg_eq E P P E" using equalityreverseE[OF `axioms`] by blast
	...
	have "seg_eq D G F H" using 5-lineE[OF `axioms` `seg_eq C G E H` `seg_eq Q D P F` `seg_eq C D E F` `bet Q C G` `bet P E H` `seg_eq Q C P E`] .
