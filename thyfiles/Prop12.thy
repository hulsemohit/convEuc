...
proof -
	...
	obtain K where "circle K C C E" using circle_f[OF `axioms` `C \<noteq> E`]  by  blast
	have "circle K C C E \<and> cir_in B K" using inside_b[OF `axioms` `circle K C C E` `bet E C F` `seg_eq C F C E` `seg_eq C E C E` `bet E B F`] .
	have "cir_in B K" using `circle K C C E \<and> cir_in B K` by blast
	obtain P Q where "col A B P \<and> col A B Q \<and> cir_on P K \<and> cir_on Q K \<and> bet P B Q" using line_circleE[OF `axioms` `circle K C C E` `cir_in B K` `A \<noteq> B`]  by  blast
	...
	have "seg_eq C P C E" using on_f[OF `axioms` `circle K C C E` `cir_on P K`] by blast
	have "seg_eq C Q C E" using on_f[OF `axioms` `circle K C C E` `cir_on Q K`] by blast
	...
	have "col P B Q" using collinear_b `axioms` `col A B P \<and> col A B Q \<and> cir_on P K \<and> cir_on Q K \<and> bet P B Q` by blast
	...
	have "ang_right P M C" using rightangle_b[OF `axioms` `bet P M Q` `seg_eq P M Q M` `seg_eq P C Q C` `M \<noteq> C`] .
	have "M = M" using equalityreflexiveE[OF `axioms`] .
	have "col C M M" using collinear_b `axioms` `M = M` by blast
	...
	have "col A B M" using `col A B M` .
	have "perp_at C M A B M" using perpat_b[OF `axioms` `col C M M` `col A B M` `col A B P` `ang_right P M C`] .
