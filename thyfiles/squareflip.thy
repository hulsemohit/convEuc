...
proof -
	...
	have "square B A D C" using square_b[OF `axioms` `seg_eq B A D C` `seg_eq B A A D` `seg_eq B A C B` `ang_right C B A` `ang_right B A D` `ang_right A D C` `ang_right D C B`] .
