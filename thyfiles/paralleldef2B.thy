...
proof -
	obtain a b c d e where "A \<noteq> B \<and> C \<noteq> D \<and> col A B a \<and> col A B b \<and> a \<noteq> b \<and> col C D c \<and> col C D d \<and> c \<noteq> d \<and> \<not> (meets A B C D) \<and> bet a e d \<and> bet c e b" sorry
	...
	proof (rule ccontr)
		...
		obtain R where "a \<noteq> b \<and> C \<noteq> D \<and> col a b R \<and> col C D R" sorry
	...
	proof (rule ccontr)
		assume "col a d P"
		have "col a e d" using col_b `axioms` `A \<noteq> B \<and> C \<noteq> D \<and> col A B a \<and> col A B b \<and> a \<noteq> b \<and> col C D c \<and> col C D d \<and> c \<noteq> d \<and> \<not> (meets A B C D) \<and> bet a e d \<and> bet c e b` by blast
		...
		have "col e b P" using col_b `axioms` `bet e b P \<and> seg_eq b P e b` by blast
		...
		proof (rule ccontr)
			assume "d = P"
			have "col c b P" using col_b `axioms` `bet c b P` by blast
			have "col c b d" sorry
			have "col b e c" using col_b `axioms` `bet b e c` by blast
		...
		have "col P b c" using col_b `axioms` `bet P b c` by blast
	...
	obtain M where "bet P M d \<and> bet a b M" using Pasch-outerE[OF `axioms` `bet P b e` `bet a e d` `\<not> col a d P`]  by  blast
	...
	have "col a b M" using col_b `axioms` `bet P M d \<and> bet a b M` by blast
	have "col A B a" using `A \<noteq> B \<and> C \<noteq> D \<and> col A B a \<and> col A B b \<and> a \<noteq> b \<and> col C D c \<and> col C D d \<and> c \<noteq> d \<and> \<not> (meets A B C D) \<and> bet a e d \<and> bet c e b` by blast
	have "col A B b" using `col A B b` .
	have "col B a b" using collinear4[OF `axioms` `col A B a` `col A B b` `A \<noteq> B`] .
	have "col b a B" using collinearorder[OF `axioms` `col B a b`] by blast
	...
	proof (rule ccontr)
		...
		qed
		hence "C = d" by blast
		have "tarski_parallel A B c C" sorry
