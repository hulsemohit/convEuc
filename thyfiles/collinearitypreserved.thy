theory collinearitypreserved
	imports Geometry betweennesspreserved congruencesymmetric congruencetransitive
begin

theorem(in euclidean_geometry) collinearitypreserved:
	assumes 
		"col A B C"
		"seg_eq A B a b"
		"seg_eq A C a c"
...
proof -
	...
	have "seg_eq C B b c" using congruencetransitive[OF `seg_eq C B B C` `seg_eq B C b c`] .
	have "seg_eq b c c b" using equalityreverseE.
	have "seg_eq C B c b" using congruencetransitive[OF `seg_eq C B b c` `seg_eq b c c b`] .
	have "seg_eq a b b a" using equalityreverseE.
	have "seg_eq A B b a" using congruencetransitive[OF `seg_eq A B a b` `seg_eq a b b a`] .
	have "seg_eq A B B A" using equalityreverseE.
	have "seg_eq B A A B" using congruencesymmetric[OF `seg_eq A B B A`] .
	...
	next
		...
		have "seg_eq a c A A" using congruencesymmetric[OF `seg_eq A A a c`] .
		have "a = c" using nullsegment1E[OF `seg_eq a c A A`] .
	...
	next
		assume "bet B A C"
		have "bet b a c" using betweennesspreserved[OF `seg_eq B A b a` `seg_eq B C b c` `seg_eq A C a c` `bet B A C`] .
		have "col a b c" using collinear_b `bet b a c` by blast
		thus ?thesis by blast
	next
		assume "bet A B C"
