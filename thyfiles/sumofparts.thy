...
proof -
	have "seg_eq A A a a" using nullsegment2E[OF `axioms`] by blast
	...
	have "seg_eq A C a c" using 5-lineE[OF `axioms` `seg_eq B C b c` `seg_eq A A a a` `seg_eq B A b a` `bet A B C` `bet a b c` `seg_eq A B a b`] .
