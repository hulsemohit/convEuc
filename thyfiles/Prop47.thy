...
theorem(in euclidean_geometry) Prop47:
	assumes 
		"triangle A B C"
		"ang_right B A C"
		"square A B F G"
...
proof -
	obtain L M where "parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A \<and> qua_eq_area A B F G B M L D" using Prop47B[OF `triangle A B C` `ang_right B A C` `square A B F G` `oppo_side G B A C` `square B C E D` `oppo_side D C B A`]  by  blast
	...
	have "square C B D E" using squareflip[OF `square B C E D`] .
	have "oppo_side D B C A" using oppositesideflip[OF `oppo_side D C B A`] .
	...
	have "tarski_parallel B C E D" using paralleldef2B[OF `parallel B C E D`] .
	have "same_side E D B C" using tarski_parallel_f[OF `tarski_parallel B C E D`] by blast
	have "oppo_side E B C A" using planeseparation[OF `same_side E D B C` `oppo_side D B C A`] .
	...
	obtain l m where "parallelogram C m l E \<and> bet C m B \<and> parallelogram m B D l \<and> bet E l D \<and> bet l m A \<and> ang_right E l A \<and> qua_eq_area A C K H C m l E" using Prop47B[OF `triangle A C B` `ang_right C A B` `square A C K H` `oppo_side H C A B` `square C B D E` `oppo_side E B C A`]  by  blast
	...
	have "col E l D" using collinear_b `parallelogram C m l E \<and> bet C m B \<and> parallelogram m B D l \<and> bet E l D \<and> bet l m A \<and> ang_right E l A \<and> qua_eq_area A C K H C m l E` by blast
	have "col D L E" using collinear_b `parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A \<and> qua_eq_area A B F G B M L D` by blast
	have "col D E L" using collinearorder[OF `col D L E`] by blast
	have "col D E l" using collinearorder[OF `col E l D`] by blast
	have "col E l L" using collinear4[OF `col D E l` `col D E L` `D \<noteq> E`] .
	have "L \<noteq> E" using betweennotequal[OF `bet D L E`] by blast
	have "E \<noteq> L" using inequalitysymmetric[OF `L \<noteq> E`] .
	have "ang_right E L A" using collinearright[OF `ang_right D L A` `col D L E` `E \<noteq> L`] .
	have "l = L" using droppedperpendicularunique[OF `ang_right E l A` `ang_right E L A` `col E l L`] .
	have "L = l" using equalitysymmetric[OF `l = L`] .
	...
	have "col L m A" using collinear_b `bet L m A` by blast
	have "col L M A" using collinear_b `parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A \<and> qua_eq_area A B F G B M L D` by blast
	have "col L A m" using collinearorder[OF `col L m A`] by blast
	have "col L A M" using collinearorder[OF `col L M A`] by blast
	have "L \<noteq> A" using betweennotequal[OF `bet L M A`] by blast
	have "col A m M" using collinear4[OF `col L A m` `col L A M` `L \<noteq> A`] .
	have "col M m A" using collinearorder[OF `col A m M`] by blast
	have "col B M C" using collinear_b `parallelogram B M L D \<and> bet B M C \<and> parallelogram M C E L \<and> bet D L E \<and> bet L M A \<and> ang_right D L A \<and> qua_eq_area A B F G B M L D` by blast
	have "col C B M" using collinearorder[OF `col B M C`] by blast
	have "col C m B" using collinear_b `parallelogram C m l E \<and> bet C m B \<and> parallelogram m B D l \<and> bet E l D \<and> bet l m A \<and> ang_right E l A \<and> qua_eq_area A C K H C m l E` by blast
	have "col C B m" using collinearorder[OF `col C m B`] by blast
	have "C \<noteq> B" using betweennotequal[OF `bet C M B`] by blast
	have "col B M m" using collinear4[OF `col C B M` `col C B m` `C \<noteq> B`] .
	have "col M m B" using collinearorder[OF `col B M m`] by blast
	have "\<not> (M \<noteq> m)"
	proof (rule ccontr)
...
hence "M \<noteq> m" by blast
		have "col m M A" using collinearorder[OF `col A m M`] by blast
		have "col m M B" using collinearorder[OF `col B M m`] by blast
		have "m \<noteq> M" using inequalitysymmetric[OF `M \<noteq> m`] .
		have "col M A B" using collinear4[OF `col m M A` `col m M B` `m \<noteq> M`] .
		have "col M B A" using collinearorder[OF `col M A B`] by blast
		have "col M B C" using collinearorder[OF `col B M C`] by blast
		have "M \<noteq> B" using betweennotequal[OF `bet C M B`] by blast
		have "col B A C" using collinear4[OF `col M B A` `col M B C` `M \<noteq> B`] .
	...
	have "qua_eq_area A C K H M C E L" using EFpermutationE[OF `qua_eq_area A C K H C M L E`] by blast
