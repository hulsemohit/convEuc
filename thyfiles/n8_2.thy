...
proof -
	...
	have "supplement A B C C D" using supplement_b[OF `axioms` `ray_on B C C` `bet A B D`] .
	have "ray_on B A A" using ray4 `axioms` `A = A` `B \<noteq> A` by blast
	have "supplement C B A A E" using supplement_b[OF `axioms` `ray_on B A A` `bet C B E`] .
	have "ang_eq A B C C B A" using ABCequalsCBA[OF `axioms` `\<not> col A B C`] .
	have "ang_eq C B D A B E" using supplements[OF `axioms` `ang_eq A B C C B A` `supplement A B C C D` `supplement C B A A E`] .
