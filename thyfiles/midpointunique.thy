...
proof -
	...
	have "bet A D C \<and> seg_eq A D D C" using midpoint_f[OF `axioms` `midpoint A D C`] .
	...
	proof (rule ccontr)
		...
		have "seg_lt A B A D" using lessthan_b[OF `axioms` `bet A B D` `seg_eq A B A B`] .
		...
		have "seg_eq C D C D" using congruencereflexiveE[OF `axioms`] by blast
		have "seg_lt C D C B" using lessthan_b[OF `axioms` `bet C D B` `seg_eq C D C D`] .
	...
	proof (rule ccontr)
		...
		have "seg_lt A D A B" using lessthan_b[OF `axioms` `bet A D B` `seg_eq A D A D`] .
		...
		have "seg_lt C B C D" using lessthan_b[OF `axioms` `bet C B D` `seg_eq C B C B`] .
