...
theorem(in euclidean_geometry) TTorder:
	assumes 
...
proof -
	...
	have "seg_sum_gt C D A B E J" using TGsymmetric[OF `seg_sum_gt A B C D E J`] .
	have "seg_sum_pair_gt C D A B E F G H" using togetherfour_b[OF `bet E F J` `seg_eq F J G H` `seg_sum_gt C D A B E J`] .
