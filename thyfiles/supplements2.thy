...
proof -
	obtain a b c d e where "supplement a b c d e \<and> ang_eq A B C a b c \<and> ang_eq P Q R d b e" using tworightangles_f[OF `axioms` `ang_sum_right A B C P Q R`]  by  blast
	obtain j k l m n where "supplement j k l m n \<and> ang_eq J K L j k l \<and> ang_eq D E F m k n" using tworightangles_f[OF `axioms` `ang_sum_right J K L D E F`]  by  blast
	...
	have "ang_eq d b e m k n" using supplements[OF `axioms` `ang_eq a b c j k l` `supplement a b c d e` `supplement j k l m n`] .
