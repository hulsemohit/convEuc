...
proof -
	...
	have "col H B K" using col_b `axioms` `bet H B K` by blast
	...
	have "L = M \<or> L = K \<or> M = K \<or> bet M L K \<or> bet L M K \<or> bet L K M" using col_f[OF `axioms` `col L M K`] .
	...
	proof (cases)
		...
		proof (rule ccontr)
			...
			have "col M B M" using col_b `axioms` `M = M` by blast
			have "col H L L" using col_b `axioms` `L = L` by blast
			have "col H L M" sorry
			have "meets M B H L" sorry
			show "False" using `meets M B H L` `\<not> (meets M B H L)` by blast
		qed
	...
	next
		...
		proof (rule ccontr)
			assume "\<not> (bet L M K)"
			have "col H B L" sorry
			have "col H L B" using collinearorder[OF `axioms` `col H B L`] by blast
			have "col M B B" using col_b `axioms` `B = B` by blast
			have "meets M B H L" sorry
			show "False" using `meets M B H L` `\<not> (meets M B H L)` by blast
		qed
		hence "bet L M K" by blast
	...
	next
		...
		proof (rule ccontr)
			...
			have "col H E L" using col_b `axioms` `bet H E L \<and> bet M E B` by blast
			have "col M E B" using col_b `axioms` `bet H E L \<and> bet M E B` by blast
	...
	next
		...
		proof (rule ccontr)
			...
			have "col H E L" using col_b `axioms` `bet H E L \<and> bet M B E` by blast
			have "col M B E" using col_b `axioms` `bet H E L \<and> bet M B E` by blast
			have "col H L E" using collinearorder[OF `axioms` `col H E L`] by blast
			have "meets M B H L" sorry
			show "False" using `meets M B H L` `\<not> (meets M B H L)` by blast
		qed
