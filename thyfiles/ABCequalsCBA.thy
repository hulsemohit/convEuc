...
proof -
	...
	proof (rule ccontr)
		assume "C = B"
		have "col C B A" using collinear_b `axioms` `C = B` by blast
	...
	have "ang_eq A B C C B A" using equalangles_b[OF `axioms` `ray_on B A E` `ray_on B C F` `ray_on B C F` `ray_on B A E` `seg_eq B E B F` `seg_eq B F B E` `seg_eq E F F E` `\<not> col A B C`] .
