...
hence "ang_lt C E A C E f" by blast
			...
			have "supplement C E A A B" using supplement_b[OF `axioms` `ray_on E A A` `bet C E B`] .
			have "supplement C E f f B" using supplement_b[OF `axioms` `ray_on E f f` `bet C E B`] .
			have "ang_lt f E B A E B" using supplementinequality[OF `axioms` `supplement C E f f B` `supplement C E A A B` `ang_lt C E A C E f`] .
...
hence "\<not> (ang_eq C E A C E f)" by blast
	...
	have "qua_eq_area A B E C F E C G" using paste3E `axioms` `tri_eq_area A E B F C E` `tri_eq_area A E C F C G` `bet B E C \<and> seg_eq B E E C` `E = E` `bet E m G \<and> bet F m C` `bet E m G \<and> bet F m C` by blast
...
hence "\<not> (ray_on E F f)" by blast
			...
			have "\<not> (meets P Q E C)" using parallel_f[OF `axioms` `parallel P Q E C`] by fastforce
