...
proof -
	...
	obtain H where "bet E H F \<and> seg_eq E H C D" using lessthan_f[OF `axioms` `seg_lt C D E F`]  by  blast
...
hence "\<not> (bet E K H)" by blast
	...
	have "seg_lt A B E F" using lessthan_b[OF `axioms` `bet E K F` `seg_eq E K A B`] .
	thus ?thesis by blast
qed
