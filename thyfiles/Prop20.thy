...
proof -
	...
	proof (rule ccontr)
		assume "B = A"
		have "col B A C" using col_b `axioms` `B = A` by blast
	...
	proof (rule ccontr)
		assume "col A D C"
		have "col B A D" using col_b `axioms` `bet B A D \<and> seg_eq A D C A` by blast
	...
	proof (rule ccontr)
		assume "D = C"
		have "col A D C" using col_b `axioms` `D = C` by blast
		show "False" using `col A D C` `\<not> col A D C` by blast
	qed
	hence "D \<noteq> C" by blast
	have "triangle A D C" sorry
	have "tri_isos A D C" sorry
	...
	proof (rule ccontr)
		assume "C = D"
		have "col A C D" using col_b `axioms` `C = D` by blast
	...
	have "seg_sum_gt B A A C B C" sorry
