...
proof -
	...
	obtain H where "col C F F \<and> col A B F \<and> col A B H \<and> ang_right H F C" sorry
	...
	next
		...
		obtain P where "bet C F P \<and> seg_eq C F P F \<and> seg_eq C A P A \<and> F \<noteq> A" sorry
	...
	next
		...
		obtain P where "bet C F P \<and> seg_eq C F P F \<and> seg_eq C B P B \<and> F \<noteq> B" sorry
	...
	obtain G where "bet D G E \<and> col A B G \<and> \<not> col A B D" sorry
	...
	have "seg_eq G B G B" using congruencereflexiveE[OF `axioms`] .
	have "col A B G" using `col A B G` .
	have "A = B \<or> A = G \<or> B = G \<or> bet B A G \<or> bet A B G \<or> bet A G B" using col_f[OF `axioms` `col A B G`] .
	...
	proof (cases)
		...
		proof (rule ccontr)
			...
			have "col A B J" using col_b `axioms` `bet B A J \<and> seg_eq A J A B` by blast
	...
	next
		...
		proof (cases)
			...
			proof (rule ccontr)
				...
				have "col B A K" using col_b `axioms` `bet A B K \<and> seg_eq B K B A` by blast
		...
		next
	next
	have "seg_eq B F B F" using congruencereflexiveE[OF `axioms`] .
	have "seg_eq A F A G" sorry
	have "col A F B" using collinearorder[OF `axioms` `col A B F`] by blast
	have "A \<noteq> B" using `A \<noteq> B` .
	have "seg_eq A F A G" using `seg_eq A F A G` .
	have "seg_eq F B F B" using congruencereflexiveE[OF `axioms`] .
	have "seg_eq F B G B" sorry
	...
	have "seg_eq F C F D" sorry
	have "bet E F C" using betweennesssymmetryE[OF `axioms` `bet C F E`] .
	have "bet E G D" using betweennesssymmetryE[OF `axioms` `bet D G E`] .
	have "bet E F D" sorry
	have "ray_on F D C" sorry
