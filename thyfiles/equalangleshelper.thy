...
proof -
	...
	have "ang_eq A B C p b q" using equalangles_b[OF `axioms` `ray_on B A U` `ray_on B C V` `ray_on b p u` `ray_on b q v` `seg_eq B U b u` `seg_eq B V b v` `seg_eq U V u v` `\<not> col A B C`] .
