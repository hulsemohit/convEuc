...
proof -
	...
	have "col B A F" using col_b `axioms` `bet B A F \<and> seg_eq A F B A` by blast
	...
	have "col C S A" using col_b `axioms` `bet E C H \<and> ang_eq E C A C A B \<and> parallel E H F B \<and> bet E S B \<and> bet C S A` by blast
	...
	have "col E S B" using col_b `axioms` `bet E C H \<and> ang_eq E C A C A B \<and> parallel E H F B \<and> bet E S B \<and> bet C S A` by blast
	have "col B S E" using collinearorder[OF `axioms` `col E S B`] by blast
	have "B = B" using equalityreflexiveE[OF `axioms`] .
	have "col B S B" using col_b `axioms` `B = B` by blast
	...
	have "col S C C" using col_b `axioms` `C = C` by blast
	have "col C S A" using col_b `axioms` `bet E C H \<and> ang_eq E C A C A B \<and> parallel E H F B \<and> bet E S B \<and> bet C S A` by blast
	...
	have "col A B M" using col_b `axioms` `bet A B M \<and> seg_eq B M A B` by blast
	...
	have "col C E H" using col_b `axioms` `bet E C H \<and> ang_eq E C A C A B \<and> parallel E H F B \<and> bet E S B \<and> bet C S A` by blast
	have "col C E E" using col_b `axioms` `E = E` by blast
	...
	have "col E S Q" using col_b `axioms` `bet A Q H \<and> bet E S Q` by blast
	...
	have "col L E H" using col_b `axioms` `bet L E H` by blast
	...
	proof (rule ccontr)
		...
		obtain c where "A \<noteq> M \<and> L \<noteq> H \<and> col A M c \<and> col L H c" sorry
		...
		have "col H E L" using col_b `axioms` `bet H E L \<and> seg_eq E L C E` by blast
		...
		have "meets E H F B" sorry
		have "\<not> (meets E H F B)" sorry
	...
	have "col A Q H" using col_b `axioms` `bet A Q H \<and> bet E S Q` by blast
	...
	have "E = E" using equalityreflexiveE[OF `axioms`] .
	have "col Q E E" using col_b `axioms` `E = E` by blast
	...
	have "col B T C" using col_b `axioms` `bet B T C \<and> bet H T Q` by blast
	...
	have "col A T A" using col_b `axioms` `A = A` by blast
	have "col A T H" using col_b `axioms` `bet A T H` by blast
	...
	have "col D R A" using col_b `axioms` `bet D R A \<and> bet H C R` by blast
	...
	have "col H C R" using col_b `axioms` `bet D R A \<and> bet H C R` by blast
	have "col R H C" using collinearorder[OF `axioms` `col H C R`] by blast
	have "R = R" using equalityreflexiveE[OF `axioms`] .
	have "col R H R" using col_b `axioms` `R = R` by blast
	...
	have "area_sum_eq C A B A B C A C D" sorry
