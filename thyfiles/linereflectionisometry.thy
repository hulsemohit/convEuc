...
proof -
	...
	have "col D B F" using collinear_b `axioms` `bet D B F` by blast
	...
	have "midpoint D M R" using midpoint_b[OF `axioms` `bet D M R` `seg_eq D M M R`] .
	...
	have "midpoint F M Q" using midpoint_b[OF `axioms` `bet F M Q` `seg_eq F M M Q`] .
	...
	have "midpoint Q A R" using midpoint_b[OF `axioms` `bet Q A R` `seg_eq Q A A R`] .
	...
	have "midpoint E A C" using midpoint_b[OF `axioms` `bet E A C` `seg_eq E A A C`] .
