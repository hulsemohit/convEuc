...
proof -
	...
	have "linear_pair G A B B D" using supplement_b[OF `axioms` `ray_on A B B` `bet G A D`] .
	...
	have "ang_suppl G A B B A C" using tworightangles_b[OF `axioms` `linear_pair G A B B D` `ang_eq G A B G A B` `ang_eq B A C B A D`] .
