...
theorem(in euclidean_geometry) Prop05:
	assumes 
		"tri_isos A B C"
	shows "ang_eq A B C A C B"
proof -
	have "triangle A B C \<and> seg_eq A B A C" using isosceles_f[OF `tri_isos A B C`] .
...
hence "col C A B" by blast
	...
	have "seg_eq C B B C \<and> ang_eq A C B A B C \<and> ang_eq A B C A C B" using Prop04[OF `seg_eq A C A B` `seg_eq A B A C` `ang_eq C A B B A C`] .
