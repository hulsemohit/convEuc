...
proof -
	...
	have "col P K H" using col_b `axioms` `bet P K H \<and> seg_eq K H P K` by blast
	have "col P K R" using col_b `axioms` `bet P K R` by blast
	...
	have "col R K K" using col_b `axioms` `K = K` by blast
	...
	have "triangle D B C" sorry
	have "parallel F K H G" sorry
	...
	have "same_side F G K H" sorry
	have "ang_suppl F K H K H G" using Prop29C[OF `axioms` `parallel K F H G` `same_side F G K H` `bet P K H`] by blast
	...
	have "ang_suppl G H M K H G" using RTcongruence[OF `axioms` `ang_suppl F K H K H G` `ang_eq F K H G H M`] .
	have "ang_suppl K H G G H M" using RTsymmetric[OF `axioms` `ang_suppl G H M K H G`] .
	...
	have "ang_suppl K H G G H M" using `ang_suppl K H G G H M` .
	have "bet K H M" using Prop14[OF `axioms` `ang_suppl K H G G H M` `ray_on H G G` `oppo_side M G H K`] by blast
	...
	have "col F K K" using col_b `axioms` `K = K` by blast
	have "col G H H" using col_b `axioms` `H = H` by blast
	have "col L M M" using col_b `axioms` `M = M` by blast
	...
	have "parallel F G K H" sorry
	have "parallel G L H M" sorry
	have "parallel F G H K" using parallelflip[OF `axioms` `parallel F G K H`] by blast
	have "col K H M" using col_b `axioms` `bet K H M` by blast
	...
	have "parallelogram F K M L" sorry
	...
	have "col B O D" using col_b `axioms` `bet B O D` by blast
	have "col B D O" using collinearorder[OF `axioms` `col B O D`] by blast
	have "\<not> col B D A" using NCorder[OF `axioms` `\<not> col A B D`] by blast
	have "oppo_side A B D C" sorry
	have "bet K H M" using `bet K H M` .
	...
	have "same_side F K G H" sorry
	have "H = H" using equalityreflexiveE[OF `axioms`] .
	have "col G H H" using col_b `axioms` `H = H` by blast
	have "oppo_side K G H M" sorry
	...
	obtain t where "bet L t F \<and> col G H t \<and> \<not> col G H L" sorry
	...
	have "col L t F" using col_b `axioms` `bet L t F \<and> col G H t \<and> \<not> col G H L` by blast
	...
	have "qua_eq_area A B C D F K M L" using paste4E[OF `axioms` `qua_eq_area A B m D F K H G` `qua_eq_area D B e C G H M L` `bet A O C` `bet B O D` `bet K H M` `bet F G L` `bet B m D` `bet B e C` `bet F j M` `bet K j L`] .
