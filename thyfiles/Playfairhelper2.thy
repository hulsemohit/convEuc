...
proof -
	...
	next
		...
		have "col E m F" using col_b `axioms` `bet A F e \<and> bet E m F` by blast
		have "col E m B" using col_b `axioms` `bet A m C \<and> bet E m B` by blast
		...
		have "E = E" using equalityreflexiveE[OF `axioms`] .
		have "col e E E" using col_b `axioms` `E = E` by blast
		have "B = B" using equalityreflexiveE[OF `axioms`] .
		have "col B B A" using col_b `axioms` `B = B` by blast
