...
proof -
	...
	have "parallel A C D B" using parallelogram_f[OF `axioms` `parallelogram A C D B`] by blast
	...
	have "parallel A B C D" using parallelogram_f[OF `axioms` `parallelogram A C D B`] by blast
	...
	have "ang_sum_right C A B A B D" using Prop29C[OF `axioms` `parallel A C B D` `same_side C D A B` `bet E A B`] by blast
	obtain p q r s t where "supplement p q r s t \<and> ang_eq C A B p q r \<and> ang_eq A B D s q t" using tworightangles_f[OF `axioms` `ang_sum_right C A B A B D`]  by  blast
	...
	have "ang_right s q t" using supplementofright[OF `axioms` `supplement p q r s t` `ang_right p q r`] by blast
	...
	have "cross A D C B" using cross_b[OF `axioms` `bet A M D` `bet C M B`] .
	...
	have "rectangle A C D B" using rectangle_b[OF `axioms` `ang_right B A C` `ang_right A C D` `ang_right C D B` `ang_right D B A` `cross A D C B`] .
