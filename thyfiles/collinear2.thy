...
proof -
	...
	proof (cases)
		...
		have "col B C A" using col_b `axioms` `B = A` by blast
	next
		case 2
	...
	next
		case 3
		have "col B C A" using col_b `axioms` `B = C` by blast
	next
		case 4
		have "col B C A" using col_b `axioms` `bet B A C` by blast
	next
		case 5
		have "bet C B A" using betweennesssymmetryE[OF `axioms` `bet A B C`] .
		have "col B C A" using col_b `axioms` `bet C B A` by blast
	next
		case 6
		have "bet B C A" using betweennesssymmetryE[OF `axioms` `bet A C B`] .
		have "col B C A" using col_b `axioms` `bet B C A` by blast
	next
