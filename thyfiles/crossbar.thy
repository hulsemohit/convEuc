...
proof -
	...
	proof (rule ccontr)
		...
		have "col B C Q" using col_b `axioms` `bet B C Q \<and> seg_eq C Q B V` by blast
	...
	obtain F where "bet A F Q \<and> bet B E F" using Pasch-outerE[OF `axioms` `bet A E C` `bet B C Q` `\<not> col B Q A`]  by  blast
	...
	proof (rule ccontr)
		...
		have "col B A P" using col_b `axioms` `bet B A P \<and> seg_eq A P B U` by blast
	...
	obtain W where "bet Q W P \<and> bet B F W" using Pasch-outerE[OF `axioms` `bet Q F A` `bet B A P` `\<not> col B P Q`]  by  blast
	...
	obtain M where "bet Q M U \<and> bet B M W" using Pasch-innerE[OF `axioms` `bet B U P` `bet Q W P` `\<not> col B P Q`] by blast
	have "bet Q M U" using `bet Q M U \<and> bet B M W` by blast
	have "bet U M Q" using betweennesssymmetryE[OF `axioms` `bet Q M U`] .
	have "\<not> (col U Q B)"
	proof (rule ccontr)
		assume "col U Q B"
		have "col B U P" using col_b `axioms` `bet B U P` by blast
	...
	obtain H where "bet U H V \<and> bet B H M" using Pasch-innerE[OF `axioms` `bet U M Q` `bet B V Q` `\<not> col U Q B`]  by  blast
