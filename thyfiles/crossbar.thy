...
proof -
	...
	have "seg_eq A P P A" using equalityreverseE[OF `axioms`] by blast
	...
	have "seg_eq P B B P" using equalityreverseE[OF `axioms`] by blast
	...
	have "seg_eq C Q Q C" using equalityreverseE[OF `axioms`] by blast
	...
	obtain M where "bet Q M U \<and> bet B M W" using Pasch-innerE[OF `axioms` `bet Q W P` `bet B U P` `\<not> col Q P B`] by blast
