...
proof -
	obtain P where "bet A a P \<and> seg_eq a P B b \<and> seg_lt C c A P" sorry
	...
	obtain L where "circle L F A a" sorry
	obtain R where "circle R G C c" sorry
	have "seg_eq G H C c" using `bet F G H \<and> seg_eq G H C c` by blast
	have "circle R G C c \<and> cir_on H R" sorry
	...
	obtain M where "bet F M H \<and> seg_eq F M D F" sorry
	...
	have "circle L F A a \<and> cir_ou H L" sorry
	have "seg_sum_gt A a B b C c" using `seg_sum_gt A a B b C c` .
	have "seg_eq D F A a" using `seg_eq D F A a` .
	have "seg_eq F G B b" using `seg_eq F G B b` .
	have "bet D F G" using `bet D F G` .
	have "seg_eq C c C c" using congruencereflexiveE[OF `axioms`] .
	...
	have "seg_eq D G G D" using equalityreverseE[OF `axioms`] .
	have "seg_lt C c G D" using lessthancongruence[OF `axioms` `seg_lt C c D G` `seg_eq D G G D`] .
	obtain N where "bet G N D \<and> seg_eq G N C c" sorry
	...
	obtain Q where "bet F Q J \<and> seg_eq F Q F G" sorry
	...
	have "circle L F A a \<and> cir_in N L" sorry
	have "circle R G C c \<and> cir_on N R" sorry
	...
	have "seg_eq F K A a" sorry
	have "seg_eq G K C c" sorry
	have "\<not> (col F G K)"
	proof (rule ccontr)
		assume "col F G K"
		have "F = G \<or> F = K \<or> G = K \<or> bet G F K \<or> bet F G K \<or> bet F K G" using col_f[OF `axioms` `col F G K`] .
		...
		next
			...
			have "seg_eq G K C c" using `seg_eq G K C c` .
			have "seg_sum_gt A a B b C c" using `seg_sum_gt A a B b C c` .
			obtain S where "bet A a S \<and> seg_eq a S B b \<and> seg_lt C c A S" sorry
		...
		next
			...
			have "seg_eq F G B b" using `seg_eq F G B b` .
			have "seg_sum_gt B b C c A a" using `seg_sum_gt B b C c A a` .
			obtain S where "bet B b S \<and> seg_eq b S C c \<and> seg_lt A a B S" sorry
		...
		next
			...
			have "seg_eq G K C c" using `seg_eq G K C c` .
			have "seg_sum_gt A a C c B b" using `seg_sum_gt A a C c B b` .
			obtain S where "bet A a S \<and> seg_eq a S C c \<and> seg_lt B b A S" sorry
