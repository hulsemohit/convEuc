...
proof -
	...
	have "col r d d" using col_b `axioms` `d = d` by blast
	...
	proof (rule ccontr)
		...
		have "col p r c" using col_b `axioms` `p = c` by blast
	...
	have "col q p p" using col_b `axioms` `p = p` by blast
	...
	have "col c p c" using col_b `axioms` `c = c` by blast
	...
	have "col p r F" using col_b `axioms` `bet q F E \<and> bet p F r` by blast
