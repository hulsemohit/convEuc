...
proof -
	...
	obtain H where "bet A H D \<and> col E F H \<and> \<not> col E F A" sorry
	...
	have "col C F D" using col_b `axioms` `bet C F D` by blast
	...
	proof (rule ccontr)
		...
		obtain G where "A \<noteq> B \<and> C \<noteq> D \<and> col A B G \<and> col C D G" sorry
		...
		have "linear_pair A E F F B" sorry
		...
		have "linear_pair D F E E C" sorry
		...
		proof (rule ccontr)
			...
			have "same_side D G E F" sorry
			have "same_side G D E F" using samesidesymmetric[OF `axioms` `same_side D G E F`] by blast
			have "F = F" using equalityreflexiveE[OF `axioms`] .
			have "col E F F" using col_b `axioms` `F = F` by blast
			have "\<not> col E F D" using `\<not> col E F D` .
			have "bet D F C" using betweennesssymmetryE[OF `axioms` `bet C F D`] .
			have "oppo_side D E F C" sorry
			have "oppo_side G E F C" using planeseparation[OF `axioms` `same_side G D E F` `oppo_side D E F C`] .
			obtain R where "bet G R C \<and> col E F R \<and> \<not> col E F G" sorry
			have "bet G R C" using `bet G R C \<and> col E F R \<and> \<not> col E F G` by blast
			have "\<not> (F \<noteq> R)"
			proof (rule ccontr)
				assume "F \<noteq> R"
				have "col G R C" using col_b `axioms` `bet G R C \<and> col E F R \<and> \<not> col E F G` by blast
				have "col C D G" using `col C D G` .
		...
		proof (rule ccontr)
			...
			obtain P where "bet G P D \<and> col E F P \<and> \<not> col E F G" sorry
			have "bet G P D" using `bet G P D \<and> col E F P \<and> \<not> col E F G` by blast
			have "col G P D" using col_b `axioms` `bet G P D \<and> col E F P \<and> \<not> col E F G` by blast
		...
		have "A = E \<or> A = G \<or> E = G \<or> bet E A G \<or> bet A E G \<or> bet A G E" using col_f[OF `axioms` `col A E G`] .
		...
		next
			...
			proof (rule ccontr)
				...
				have "col D G F" using collinear4[OF `axioms` `col C D G` `col C D F` `C \<noteq> D`] .
				have "col D A F" sorry
				have "col A H D" using col_b `axioms` `bet A H D \<and> col E F H \<and> \<not> col E F A` by blast
		...
		next
			...
			qed
			hence "E = F" by blast
			have "col E F A" using col_b `axioms` `E = F` by blast
			have "\<not> (meets A B C D)"
			proof (rule ccontr)
				assume "meets A B C D"
				have "\<not> col E F A" using `\<not> col E F A` .
	...
	have "col A B E" using col_b `axioms` `bet A E B` by blast
	have "D = D" using equalityreflexiveE[OF `axioms`] .
	have "col C D D" using col_b `axioms` `D = D` by blast
	have "col C D F" using col_b `axioms` `bet C F D` by blast
	have "A \<noteq> E" using betweennotequal[OF `axioms` `bet A E B`] by blast
	have "F \<noteq> D" using betweennotequal[OF `axioms` `bet C F D`] by blast
