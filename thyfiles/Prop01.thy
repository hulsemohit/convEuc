...
proof -
	...
	obtain C where "cir_on C K \<and> cir_on C J" using circle_circleE[OF `axioms` `circle K B A B` `cir_in B K` `cir_ou D K` `circle J A A B` `cir_on B J` `cir_on D J`]  by  blast
