...
theorem(in euclidean_geometry) parallelflip:
	assumes 
		"parallel A B C D"
...
hence "meets A B D C" by blast
		obtain P where "A \<noteq> B \<and> D \<noteq> C \<and> col A B P \<and> col D C P" using meet_f[OF `meets A B D C`]  by  blast
...
hence "meets B A C D" by blast
		obtain P where "B \<noteq> A \<and> C \<noteq> D \<and> col B A P \<and> col C D P" using meet_f[OF `meets B A C D`]  by  blast
...
hence "meets B A D C" by blast
		obtain P where "B \<noteq> A \<and> D \<noteq> C \<and> col B A P \<and> col D C P" using meet_f[OF `meets B A D C`]  by  blast
	...
	have "parallel B A C D" using parallel_b[OF `B \<noteq> A` `C \<noteq> D` `col B A a` `col B A b` `a \<noteq> b` `col C D c` `col C D d` `c \<noteq> d` `\<not> (meets B A C D)` `bet a M d` `bet c M b`] .
	...
	have "parallel A B D C" using parallel_b[OF `A \<noteq> B` `D \<noteq> C` `col A B a` `col A B b` `a \<noteq> b` `col D C c` `col D C d` `c \<noteq> d` `\<not> (meets A B D C)` `bet a M d` `bet c M b`] .
	...
	have "parallel B A D C" using parallel_b[OF `B \<noteq> A` `D \<noteq> C` `col B A a` `col B A b` `a \<noteq> b` `col D C c` `col D C d` `c \<noteq> d` `\<not> (meets B A D C)` `bet a M d` `bet c M b`] .
