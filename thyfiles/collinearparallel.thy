...
proof -
	...
	proof (rule ccontr)
		...
		have "meets A B c d" using meet_b[OF `axioms` `A \<noteq> B` `c \<noteq> d` `col A B E` `col c d E`] .
	...
	have "parallel A B C d" using parallel_b[OF `axioms` `A \<noteq> B` `C \<noteq> d` `col A B a` `col A B b` `a \<noteq> b` `col C d p` `col C d q` `p \<noteq> q` `\<not> (meets A B C d)` `bet a R q` `bet p R b`] .
