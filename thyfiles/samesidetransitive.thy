...
theorem(in euclidean_geometry) samesidetransitive:
	assumes 
		"same_side P Q A B"
		"same_side Q R A B"
	shows "same_side P R A B"
proof -
	obtain E F G where "col A B E \<and> col A B F \<and> bet Q E G \<and> bet R F G \<and> \<not> col A B Q \<and> \<not> col A B R" using sameside_f[OF `same_side Q R A B`]  by  blast
	...
	have "oppo_side Q A B G" using oppositeside_b[OF `bet Q E G` `col A B E` `\<not> col A B Q`] .
	have "oppo_side P A B G" using planeseparation[OF `same_side P Q A B` `oppo_side Q A B G`] .
	obtain M where "bet P M G \<and> col A B M \<and> \<not> col A B P" using oppositeside_f[OF `oppo_side P A B G`]  by  blast
	...
	have "same_side P R A B" using sameside_b[OF `col A B M` `col A B F` `bet P M G` `bet R F G` `\<not> col A B P` `\<not> col A B R`] .
