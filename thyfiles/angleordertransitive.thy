...
proof -
	obtain U V W where "bet U W V \<and> ray_on Q P U \<and> ray_on Q R V \<and> ang_eq D E F P Q W" sorry
	...
	have "ang_eq P Q W D E F" using equalanglessymmetric[OF `axioms` `ang_eq D E F P Q W`] .
	have "Q \<noteq> W" using angledistinct[OF `axioms` `ang_eq D E F P Q W`] by blast
	...
	obtain H S T where "bet S H T \<and> ray_on Q U S \<and> ray_on Q W T \<and> ang_eq A B C U Q H" sorry
	...
	have "Q \<noteq> H" using angledistinct[OF `axioms` `ang_eq A B C U Q H`] by blast
	have "H = H" using equalityreflexiveE[OF `axioms`] .
	have "ray_on Q H H" using ray4 `axioms` `H = H` `Q \<noteq> H` by blast
	have "ang_eq A B C P Q H" using equalangleshelper[OF `axioms` `ang_eq A B C U Q H` `ray_on Q U P` `ray_on Q H H`] .
	...
	have "ang_eq D E F P Q T" using equalangleshelper[OF `axioms` `ang_eq D E F U Q W` `ray_on Q U P` `ray_on Q W T`] .
	have "\<not> col P Q T" using equalanglesNC[OF `axioms` `ang_eq D E F P Q T`] .
	have "bet S H T" using `bet S H T \<and> ray_on Q U S \<and> ray_on Q W T \<and> ang_eq A B C U Q H` by blast
	have "ray_on Q P U" using `ray_on Q P U` .
	have "Q \<noteq> P" using ray2[OF `axioms` `ray_on Q P U`] .
	...
	proof (rule ccontr)
		...
		have "ray_on Q P U" using `ray_on Q P U` .
		have "col Q P U" using rayimpliescollinear[OF `axioms` `ray_on Q P U`] .
		have "col U Q P" using collinearorder[OF `axioms` `col Q P U`] by blast
		have "Q \<noteq> U" using raystrict[OF `axioms` `ray_on Q P U`] .
		have "U \<noteq> Q" using inequalitysymmetric[OF `axioms` `Q \<noteq> U`] .
		have "col Q S P" using collinear4[OF `axioms` `col U Q S` `col U Q P` `U \<noteq> Q`] .
		have "col S Q P" using collinearorder[OF `axioms` `col Q S P`] by blast
		have "Q \<noteq> S" using raystrict[OF `axioms` `ray_on Q U S`] .
		have "S \<noteq> Q" using inequalitysymmetric[OF `axioms` `Q \<noteq> S`] .
		have "col Q T P" using collinear4[OF `axioms` `col S Q T` `col S Q P` `S \<noteq> Q`] .
		have "col P Q T" using collinearorder[OF `axioms` `col Q T P`] by blast
		show "False" using `col P Q T` `\<not> col P Q T` by blast
	qed
	hence "\<not> col S Q T" by blast
	have "triangle S Q T" sorry
	have "ray_on Q S U" using ray5[OF `axioms` `ray_on Q U S`] .
	have "ray_on Q T W" using `ray_on Q T W` .
	obtain K where "ray_on Q H K \<and> bet U K W" using crossbar[OF `axioms` `triangle S Q T` `bet S H T` `ray_on Q S U` `ray_on Q T W`]  by  blast
	...
	have "bet U K V" using n3_6b[OF `axioms` `bet U K W` `bet U W V`] .
	...
	have "ang_eq A B C P Q K" using equalangleshelper[OF `axioms` `ang_eq A B C P Q H` `ray_on Q P P` `ray_on Q H K`] .
	have "ang_lt A B C P Q R" sorry
