...
proof -
	...
	obtain D where "bet C B D \<and> seg_eq C B D B \<and> seg_eq C A D A \<and> B \<noteq> A" sorry
	...
	proof (rule ccontr)
		assume "col A B D"
		have "col C B D" using col_b `axioms` `bet C B D \<and> seg_eq C B D B \<and> seg_eq C A D A \<and> B \<noteq> A` by blast
