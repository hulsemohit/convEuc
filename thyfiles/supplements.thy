...
proof -
	have "bet A B F" using supplement_f[OF `axioms` `supplement A B C D F`] by blast
	have "ray_on B C D" using supplement_f[OF `axioms` `supplement A B C D F`] by blast
	have "bet a b f" using supplement_f[OF `axioms` `supplement a b c d f`] by blast
	have "ray_on b c d" using supplement_f[OF `axioms` `supplement a b c d f`] by blast
	...
	have "bet u b w" using `bet u b w` .
	have "seg_eq V W v w" using n5_lineE[OF `axioms` `seg_eq B W b w` `seg_eq U V u v` `seg_eq B V b v` `bet U B W` `bet u b w` `seg_eq U B u b`] .
	have "bet U B W" using `bet U B W` .
	have "bet A B F" using `bet A B F` .
