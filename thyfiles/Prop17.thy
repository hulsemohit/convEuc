...
proof -
	...
	have "col B C D" using col_b `axioms` `bet B C D \<and> seg_eq C D B C` by blast
	...
	obtain a d e where "bet a e d \<and> ray_on C A a \<and> ray_on C D d \<and> ang_eq A B C A C e" sorry
	...
	have "col C A C" using col_b `axioms` `C = C` by blast
	...
	have "col A E D" using col_b `axioms` `ray_on C e E \<and> bet A E D` by blast
	...
	have "col A F C" using col_b `axioms` `bet A F C \<and> bet B F E` by blast
	...
	have "area_sum_eq A B C B C A E C B" sorry
