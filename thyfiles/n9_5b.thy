...
theorem(in euclidean_geometry) n9_5b:
	assumes 
		"oppo_side P A B C"
		"bet R Q P"
		"\<not> col C P R"
...
proof -
	...
	obtain F where "bet C F Q \<and> bet R F S" using Pasch_innerE[OF `bet C S P` `bet R Q P` `\<not> col C P R`]  by  blast
	have "bet C F Q" using `bet C F Q \<and> bet R F S` by blast
	have "bet R F S" using `bet C F Q \<and> bet R F S` by blast
	have "col R S F" using collinear_b `bet C F Q \<and> bet R F S` by blast
	have "\<not> (A = B)"
	proof (rule ccontr)
	...
	have "R \<noteq> S" using betweennotequal[OF `bet R F S`] by blast
...
hence "col A B Q" by blast
		...
		have "col C F Q" using collinear_b `bet C F Q \<and> bet R F S` by blast
