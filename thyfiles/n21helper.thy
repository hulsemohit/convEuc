...
proof -
	...
	have "bet B A H" using `bet B A H \<and> seg_eq A H A E \<and> seg_lt B E B H` by simp
	have "B \<noteq> A" using betweennotequal[OF `axioms` `bet B A H`] by blast
	have "seg_eq A H A E" using `bet B A H \<and> seg_eq A H A E \<and> seg_lt B E B H` by simp
	have "seg_lt B E B H" using `bet B A H \<and> seg_eq A H A E \<and> seg_lt B E B H` by simp
	have "\<not> (B = E)"
	proof (rule ccontr)
		assume "B = E"
		have "seg_lt B B B H" sorry
		obtain K where "bet B K H \<and> seg_eq B K B B" sorry
		have "bet B K H" using `bet B K H \<and> seg_eq B K B B` by simp
		have "seg_eq B K B B" using `bet B K H \<and> seg_eq B K B B` by simp
	...
	have "bet B A F" using `bet B A F \<and> seg_eq A F A C` by simp
	have "seg_eq A F A C" using `bet B A F \<and> seg_eq A F A C` by simp
	...
	have "seg_eq E G E C" using `bet B E G \<and> seg_eq E G E C` by simp
	...
	have "bet B E G" using `bet B E G \<and> seg_eq E G E C` by simp
