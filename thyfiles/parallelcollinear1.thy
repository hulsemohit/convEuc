...
hence "meets C d J r" by blast
	...
	have "col p c c" using collinear_b `axioms` `c = c` by blast
	have "q \<noteq> c" using betweennotequal[OF `axioms` `bet q E c`] by blast
	...
	have "col r p F" using collinear_b `axioms` `bet q F C \<and> bet r p F` by blast
	...
	have "col q F C" using collinear_b `axioms` `bet q F C \<and> bet r p F` by blast
	have "col q C F" using collinearorder[OF `axioms` `col q F C`] by blast
	have "C = C" using equalityreflexiveE[OF `axioms`] .
	have "col q C C" using collinear_b `axioms` `C = C` by blast
