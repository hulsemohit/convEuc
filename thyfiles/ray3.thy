...
theorem(in euclidean_geometry) ray3:
	assumes 
		"ray_on B C D"
		"ray_on B C V"
	shows "ray_on B D V"
proof -
	obtain E where "bet E B D \<and> bet E B C" using ray_f[OF `ray_on B C D`]  by  blast
	obtain H where "bet H B V \<and> bet H B C" using ray_f[OF `ray_on B C V`]  by  blast
...
hence "bet B E H" by blast
			have "bet H E B" using betweennesssymmetryE[OF `bet B E H`] .
			have "bet E B V" using n3_6a[OF `bet H E B` `bet H B V`] .
...
hence "bet B H E" by blast
			...
			have "bet E B V" using n3_7a[OF `bet E H B` `bet H B V`] .
			show "False" using `bet E B V` `\<not> (bet E B V)` by blast
		qed
		hence "\<not> (bet B H E)" by blast
		have "bet C B E" using betweennesssymmetryE[OF `bet E B C`] .
		have "bet C B H" using betweennesssymmetryE[OF `bet H B C`] .
		have "H = E" using outerconnectivity[OF `bet C B H` `bet C B E` `\<not> (bet B H E)` `\<not> (bet B E H)`] .
		have "bet E B V" using `bet H B V` `H = E` by blast
		show "False" using `bet E B V` `\<not> (bet E B V)` by blast
	qed
	hence "bet E B V" by blast
	have "ray_on B D V" using ray_b[OF `bet E B V` `bet E B D`] .
