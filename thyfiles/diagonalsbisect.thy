...
proof -
	...
	have "ray_on C D D" sorry
	have "ray_on C A M" sorry
	...
	have "ray_on D C C" sorry
	have "ray_on D B M" sorry
	...
	have "seg_eq M A M C" using `seg_eq M A M C \<and> seg_eq M B M D \<and> ang_eq A M B C M D` by simp
	have "seg_eq M B M D" using `seg_eq M A M C \<and> seg_eq M B M D \<and> ang_eq A M B C M D` by simp
	...
	have "midpoint A M C \<and> midpoint B M D"  using `midpoint A M C` `midpoint B M D` by simp
