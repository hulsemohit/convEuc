...
proof -
	...
	have "supplement C B A A D" using supplement_b[OF `axioms` `ray_on B A A` `bet C B D`] .
	...
	have "ang_sum_right C B A A B D" using tworightangles_b[OF `axioms` `supplement C B A A D` `ang_eq C B A C B A` `ang_eq A B D A B D`] .
