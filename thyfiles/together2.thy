...
theorem together2:
	...
	shows: "ray_on M F N"
proof -
	obtain J where "bet A a J \<and> seg_eq a J C c \<and> seg_lt B b A J" sorry
	...
	proof (rule ccontr)
		...
		have "ray_on F G N" sorry
		have "ray_on G F M" sorry
		have "bet F M G" using tworays[OF `axioms` `ray_on F G M` `ray_on G F M`] .
		have "seg_eq G M C c" sorry
	...
	have "M = F \<or> M = N \<or> F = N \<or> bet F M N \<or> bet M F N \<or> bet M N F" using col_f[OF `axioms` `col M F N`] .
