...
theorem(in euclidean_geometry) together2:
	assumes 
	...
	shows "ray_on M F N"
proof -
	obtain J where "bet A a J \<and> seg_eq a J C c \<and> seg_lt B b A J" using togethergreater_f[OF `seg_sum_gt A a C c B b`]  by  blast
	...
	have "a \<noteq> J" using betweennotequal[OF `bet A a J`] by blast
	have "C \<noteq> c" using nullsegment3[OF `a \<noteq> J` `seg_eq a J C c`] .
	have "\<not> (M = N)"
	proof (rule ccontr)
		...
		have "bet F M G" using tworays[OF `ray_on F G M` `ray_on G F M`] .
		have "seg_eq G M C c" using `seg_eq G N C c` `M = N` by blast
		have "seg_eq M G C c" using congruenceflip[OF `seg_eq G M C c`] by blast
		have "seg_lt F G F G" using together[OF `seg_sum_gt A a C c B b` `seg_eq F M A a` `seg_eq M G C c` `bet F M G` `seg_eq F G B b`] by blast
	...
	have "F \<noteq> M" using raystrict[OF `ray_on F G M`] .
	have "M \<noteq> F" using inequalitysymmetric[OF `F \<noteq> M`] .
	obtain D where "bet M F D \<and> seg_eq F D M F" using extensionE[OF `M \<noteq> F` `M \<noteq> F`]  by  blast
	have "bet M F D" using `bet M F D \<and> seg_eq F D M F` by blast
	have "ray_on F G M" using `ray_on F G M` .
	have "bet F M G \<or> G = M \<or> bet F G M" using ray1[OF `ray_on F G M`] .
	consider "bet F M G"|"G = M"|"bet F G M" using `bet F M G \<or> G = M \<or> bet F G M`  by blast
	hence "bet G F D"
	proof (cases)
		assume "bet F M G"
		have "bet G M F" using betweennesssymmetryE[OF `bet F M G`] .
		have "bet M F D" using `bet M F D` .
		have "bet G F D" using n3_7a[OF `bet G M F` `bet M F D`] .
	...
	next
		assume "bet F G M"
		have "bet M G F" using betweennesssymmetryE[OF `bet F G M`] .
		have "bet M F D" using `bet M F D` .
		have "bet G F D" using n3_6a[OF `bet M G F` `bet M F D`] .
		thus ?thesis by blast
	qed
	have "bet D F M" using betweennesssymmetryE[OF `bet M F D`] .
...
hence "bet F M N" by blast
		...
		obtain P where "bet F M P \<and> seg_eq M P C c" using extensionE[OF `F \<noteq> M` `C \<noteq> c`]  by  blast
		...
		have "seg_lt F G F P" using together[OF `seg_sum_gt A a C c B b` `seg_eq F M A a` `seg_eq M P C c` `bet F M P` `seg_eq F G B b`] by blast
		have "seg_eq C c G N" using congruencesymmetric[OF `seg_eq G N C c`] .
		have "seg_eq C c N G" using congruenceflip[OF `seg_eq C c G N`] by blast
		have "seg_eq M P N G" using congruencetransitive[OF `seg_eq M P C c` `seg_eq C c N G`] .
		have "seg_eq F M F M" using congruencereflexiveE.
		have "seg_lt F M F N" using lessthan_b[OF `bet F M N` `seg_eq F M F M`] .
...
hence "bet F N G" by blast
			have "seg_lt F P F G" using lessthanadditive[OF `seg_lt F M F N` `bet F M P` `bet F N G` `seg_eq M P N G`] .
			have "seg_lt F G F P" using `seg_lt F G F P` .
			have "seg_lt F G F G" using lessthantransitive[OF `seg_lt F G F P` `seg_lt F P F G`] .
			have "\<not> (seg_lt F G F G)" using trichotomy2[OF `seg_lt F G F G`] .
			show "False" using `\<not> (seg_lt F G F G)` `seg_lt F G F G` by blast
		qed
...
hence "\<not> (bet G F N)" by blast
		...
		have "bet N F G" using betweennesssymmetryE[OF `bet G F N`] .
...
hence "bet F M G" by blast
				...
				have "bet N F M" using innertransitivityE[OF `bet N F G` `bet F M G`] .
...
hence "bet F G M" by blast
				have "bet N F G" using `bet N F G` .
				have "bet N F M" using n3_7b[OF `bet N F G` `bet F G M`] .
				show "False" using `bet N F M` `\<not> (bet N F M)` by blast
			qed
			hence "\<not> (bet F G M)" by blast
			have "G = M" using outerconnectivity[OF `bet D F G` `bet D F M` `\<not> (bet F G M)` `\<not> (bet F M G)`] .
			have "bet N F M" using `bet N F G` `G = M` by blast
			show "False" using `bet N F M` `\<not> (bet N F M)` by blast
		qed
		hence "bet N F M" by blast
		have "bet F M N" using `bet F M N` .
		have "bet N F N" using n3_7b[OF `bet N F M` `bet F M N`] .
		have "\<not> (bet N F N)" using betweennessidentityE.
		show "False" using `\<not> (bet N F N)` `bet N F N` by blast
	qed
	hence "\<not> (bet F M N)" by blast
	have "col G F N" using rayimpliescollinear[OF `ray_on G F N`] .
	have "col F G M" using rayimpliescollinear[OF `ray_on F G M`] .
	have "col G F M" using collinearorder[OF `col F G M`] by blast
	have "F \<noteq> G" using betweennotequal[OF `bet D F G`] by blast
	have "G \<noteq> F" using inequalitysymmetric[OF `F \<noteq> G`] .
	have "col F N M" using collinear4[OF `col G F N` `col G F M` `G \<noteq> F`] .
	have "col M F N" using collinearorder[OF `col F N M`] by blast
	have "M = F \<or> M = N \<or> F = N \<or> bet F M N \<or> bet M F N \<or> bet M N F" using collinear_f[OF `col M F N`] .
...
hence "\<not> (ray_on M F N)" by blast
	...
	next
		assume "F = N"
		have "N = F" using equalitysymmetric[OF `F = N`] .
		have "ray_on M F N" using ray4 `N = F` `M \<noteq> F` by blast
		thus ?thesis by blast
	next
		assume "bet F M N"
		have "\<not> (\<not> (ray_on M F N))"
...
hence "\<not> (ray_on M F N)" by blast
	...
	next
		assume "bet M F N"
		have "ray_on M F N" using ray4 `bet M F N` `M \<noteq> F` by blast
		thus ?thesis by blast
	next
		assume "bet M N F"
		have "ray_on M F N" using ray4 `bet M N F` `M \<noteq> F` by blast
		thus ?thesis by blast
	qed
	thus ?thesis by blast
qed
