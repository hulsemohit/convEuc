...
proof -
	obtain a b c d e where "linear_pair a b c d e \<and> ang_eq A B C a b c \<and> ang_eq D B E d b e" sorry
	...
	have "linear_pair A B C D T" sorry
	have "ang_eq a b c A B C" using equalanglessymmetric[OF `axioms` `ang_eq A B C a b c`] .
	have "ang_eq d b e D B E" using equalanglessymmetric[OF `axioms` `ang_eq D B E d b e`] .
	...
	have "col A B T" using col_b `axioms` `bet A B T \<and> seg_eq B T B E` by blast
	...
	obtain m where "bet A m E \<and> col D B m \<and> \<not> col D B A" sorry
	...
	have "linear_pair A B C D E" sorry
