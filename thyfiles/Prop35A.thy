...
proof -
	...
	obtain e where "bet F e A \<and> seg_eq F e F E" sorry
	...
	proof (rule ccontr)
		assume "A = D"
		have "col A D C" using col_b `axioms` `A = D` by blast
	...
	next
		...
		have "seg_eq A E E F" sorry
		have "seg_eq A E D F" sorry
	next
	...
	have "tri_cong F D C E A B" sorry
	...
	proof (cases)
		...
		have "col A D A" using col_b `axioms` `A = A` by blast
		...
		have "col A M H" using col_b `axioms` `bet B H E \<and> bet A M H` by blast
		...
		proof (rule ccontr)
			...
			obtain q where "E \<noteq> A \<and> C \<noteq> B \<and> col E A q \<and> col C B q" sorry
		...
		have "col E A A" using col_b `axioms` `A = A` by blast
		...
		have "bet E H B" using `bet E H B` .
		have "bet E G B" using n3_6b[OF `axioms` `bet E G H` `bet E H B`] .
		have "col E G B" using col_b `axioms` `bet E G B` by blast
		have "\<not> (col D E G)"
		proof (rule ccontr)
			assume "col D E G"
			...
			have "col E D D" using col_b `axioms` `D = D` by blast
		...
		have "col E G G" using col_b `axioms` `G = G` by blast
		...
		have "col C G D" using col_b `axioms` `bet C G D \<and> bet E G H` by blast
		have "col D G C" using collinearorder[OF `axioms` `col C G D`] by blast
		have "col D G G" using col_b `axioms` `G = G` by blast
	...
	next
		...
		proof (rule ccontr)
			assume "col B E D"
			have "col A E D" using col_b `axioms` `bet A E D` by blast
	...
	next
		...
		have "tri_eq_area B E C C D B" sorry
		have "parallelogram A B C E" sorry
		...
		have "col E M B" using col_b `axioms` `bet E M B` by blast
		have "col B E M" using collinearorder[OF `axioms` `col E M B`] by blast
		have "parallel A E B C" sorry
		...
		have "oppo_side A B E C" sorry
		have "parallelogram D B C F" sorry
		...
		have "col D m C" using col_b `axioms` `bet D m C \<and> bet B m F` by blast
		have "col C D m" using collinearorder[OF `axioms` `col D m C`] by blast
		have "oppo_side F C D B" sorry
		...
		have "qua_eq_area B A E C E B C F" sorry
		...
		have "qua_eq_area E B C F A B C D" sorry
		have "qua_eq_area A B C D E B C F" using EFsymmetricE[OF `axioms` `qua_eq_area E B C F A B C D`] .
	next
