...
proof -
	...
	have "col B M C" using collinear_b `axioms` `bet A M D \<and> bet B M C` by blast
	have "col B m C" using collinear_b `axioms` `bet A m E \<and> bet B m C` by blast
	...
	have "oppo_side E C B A" using oppositeside_b[OF `axioms` `bet E m A` `col C B m` `\<not> col C B E`] .
	have "oppo_side D C B A" using oppositeside_b[OF `axioms` `bet D M A` `col C B M` `\<not> col C B D`] .
	...
	have "same_side E D C B" using sameside_b[OF `axioms` `col C B m` `col C B M` `bet E m A` `bet D M A` `\<not> col C B E` `\<not> col C B D`] .
