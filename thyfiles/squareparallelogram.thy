...
proof -
	...
	have "col D A R" using col_b `axioms` `bet D A R \<and> seg_eq A R D A` by blast
	...
	have "seg_eq A B c E \<and> seg_eq A B B c \<and> seg_eq A B E A \<and> ang_right E A B \<and> ang_right A B c \<and> ang_right B c E \<and> ang_right c E A" sorry
	...
	have "col R A D" using col_b `axioms` `bet R A D` by blast
	...
	have "seg_eq A B C D" sorry
	have "square A B c D" sorry
	have "seg_eq A B c D" sorry
	...
	have "bet A m c \<and> seg_eq A m m c" sorry
	have "bet B m D \<and> seg_eq B m m D" sorry
	...
	have "col A m c" using col_b `axioms` `bet A m c \<and> seg_eq A m m c` by blast
	...
	proof (rule ccontr)
		assume "col C D m"
		have "col B m D" using col_b `axioms` `bet B m D \<and> seg_eq B m m D` by blast
