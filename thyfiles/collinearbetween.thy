...
proof -
	...
	proof (rule ccontr)
		...
		have "col A H B" sorry
		have "col H A B" using collinearorder[OF `axioms` `col A H B`] by blast
		have "col A H D" using col_b `axioms` `bet A H D` by blast
	...
	proof (rule ccontr)
		...
		have "col E Q A" using col_b `axioms` `bet E Q A \<and> bet D F Q` by blast
		have "col D F Q" using col_b `axioms` `bet E Q A \<and> bet D F Q` by blast
	...
	proof (rule ccontr)
		...
		have "col F R D" using col_b `axioms` `bet F R D \<and> bet A E R` by blast
		have "col A E R" using col_b `axioms` `bet F R D \<and> bet A E R` by blast
	...
	have "E = F \<or> E = H \<or> F = H \<or> bet F E H \<or> bet E F H \<or> bet E H F" using col_f[OF `axioms` `col E F H`] .
