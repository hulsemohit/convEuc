...
hence "col A B F" by blast
	...
	obtain J where "col F H H \<and> col A B H \<and> col A B J \<and> ang_right J H F" using perpat_f[OF `axioms` `perp_at F H A B H`]  by  blast
	...
	proof (rule ccontr)
		...
		hence "J = H" by blast
		have "col J H F" using collinear_b `axioms` `J = H` by blast
	...
	have "col J H T" using collinear_b `axioms` `bet J H T \<and> seg_eq H T H J` by blast
...
hence "col J T P" by blast
	...
	proof (rule ccontr)
		...
		hence "H = Q" by blast
		have "col J H Q" using collinear_b `axioms` `H = Q` by blast
	...
	proof (rule ccontr)
		...
		hence "H = F" by blast
		have "col J H F" using collinear_b `axioms` `H = F` by blast
		show "False" using `col J H F` `\<not> col J H F` by blast
	qed
...
hence "col H A S" by blast
		...
		proof (cases)
			assume "G = H"
			have "col G H A" using collinear_b `axioms` `G = H` by blast
			thus ?thesis by blast
		next
			assume "G \<noteq> H"
		...
		have "G = H \<or> G = A \<or> H = A \<or> bet H G A \<or> bet G H A \<or> bet G A H" using collinear_f[OF `axioms` `col G H A`] .
...
hence "col S A H" by blast
		...
		next
			...
			have "supplement H A F F G" using supplement_b[OF `axioms` `ray_on A F F` `bet H A G`] .
			have "supplement H A S S G" using supplement_b[OF `axioms` `ray_on A S S` `bet H A G`] .
			...
			have "ang_eq F A G S A G" using supplements[OF `axioms` `ang_eq H A F H A S` `supplement H A F F G` `supplement H A S S G`] .
			thus ?thesis by blast
		qed
		thus ?thesis by blast
	qed
	...
	obtain M where "bet S M P \<and> col J T M \<and> \<not> col J T S" using oppositeside_f[OF `axioms` `oppo_side S J T P`]  by  blast
...
hence "col A B S" by blast
	...
	have "oppo_side S A B P" using oppositeside_b[OF `axioms` `bet S M P` `col A B M` `\<not> col A B S`] .
