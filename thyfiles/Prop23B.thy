...
proof -
	...
	next
		...
		have "seg_eq H A H A" using congruencereflexiveE[OF `axioms`] by blast
		have "\<not> (col S H A)"
		proof (rule ccontr)
			assume "col S H A"
		...
		next
			...
			have "linear_pair H A F F G" using supplement_b[OF `axioms` `ray_on A F F` `bet H A G`] .
			have "linear_pair H A S S G" using supplement_b[OF `axioms` `ray_on A S S` `bet H A G`] .
