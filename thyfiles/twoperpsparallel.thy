...
proof -
	...
	have "supplement B C D D E" using supplement_b[OF `axioms` `ray_on C D D` `bet B C E`] .
	...
	have "ang_sum_right A B C B C D" using tworightangles_b[OF `axioms` `supplement B C D D E` `ang_eq A B C B C D` `ang_eq B C D D C E`] .
	have "parallel B A C D" using Prop28C[OF `axioms` `ang_sum_right A B C B C D` `same_side A D B C`] .
