...
theorem(in euclidean_geometry) rectangleparallelogram:
	assumes 
		"rectangle A B C D"
...
hence "meets A B C D" by blast
		obtain P where "A \<noteq> B \<and> C \<noteq> D \<and> col A B P \<and> col C D P" using meet_f[OF `meets A B C D`]  by  blast
		...
		proof (rule ccontr)
			...
			have "col D A B" using collinearorder[OF `col A B D`] by blast
		...
		have "ang_right P A D" using collinearright[OF `ang_right B A D` `col B A P` `P \<noteq> A`] .
		have "P \<noteq> D" using inequalitysymmetric[OF `D \<noteq> P`] .
		have "ang_right P D A" using collinearright[OF `ang_right C D A` `col C D P` `P \<noteq> D`] .
		have "ang_right A D P" using n8_2[OF `ang_right P D A`] .
		have "\<not> (ang_right P A D)" using n8_7[OF `ang_right A D P`] .
	...
	have "C \<noteq> D" using NCdistinct[OF `\<not> col C D A`] by blast
	...
	have "parallel A B C D" using parallel_b[OF `A \<noteq> B` `C \<noteq> D` `col A B A` `col A B B` `A \<noteq> B` `col C D D` `col C D C` `D \<noteq> C` `\<not> (meets A B C D)` `bet A M C` `bet D M B`] .
	have "\<not> (meets A D B C)"
	proof (rule ccontr)
		assume "\<not> (\<not> (meets A D B C))"
hence "meets A D B C" by blast
		obtain P where "A \<noteq> D \<and> B \<noteq> C \<and> col A D P \<and> col B C P" using meet_f[OF `meets A D B C`]  by  blast
		...
		have "P \<noteq> A" using inequalitysymmetric[OF `A \<noteq> P`] .
		have "col D A P" using collinearorder[OF `col A D P`] by blast
		have "ang_right P A B" using collinearright[OF `ang_right D A B` `col D A P` `P \<noteq> A`] .
		...
		have "ang_right P B A" using collinearright[OF `ang_right C B A` `col C B P` `P \<noteq> B`] .
		have "ang_right B A P" using n8_2[OF `ang_right P A B`] .
		have "\<not> (ang_right P B A)" using n8_7[OF `ang_right B A P`] .
		show "False" using `\<not> (ang_right P B A)` `ang_right P B A` by blast
	qed
	hence "\<not> (meets A D B C)" by blast
	have "A \<noteq> D" using NCdistinct[OF `\<not> col C D A`] by blast
	...
	have "bet A M C" using `bet A M C` .
	have "parallel A D B C" using parallel_b[OF `A \<noteq> D` `B \<noteq> C` `col A D A` `col A D D` `A \<noteq> D` `col B C B` `col B C C` `B \<noteq> C` `\<not> (meets A D B C)` `bet A M C` `bet B M D`] .
