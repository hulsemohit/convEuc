...
theorem(in euclidean_geometry) tarskiparallelflip:
	assumes 
...
hence "meets A B D C" by blast
		obtain T where "A \<noteq> B \<and> D \<noteq> C \<and> col A B T \<and> col D C T" using meet_f[OF `meets A B D C`]  by  blast
		...
		have "col C D T" using collinearorder[OF `col D C T`] by blast
		...
		have "meets A B C D" using meet_b[OF `A \<noteq> B` `C \<noteq> D` `col A B T` `col C D T`] .
	...
	have "tarski_parallel A B D C" using tarski_parallel_b[OF `A \<noteq> B` `D \<noteq> C` `\<not> (meets A B D C)` `same_side D C A B`] .
	have "\<not> (meets B A C D)"
	proof (rule ccontr)
		assume "\<not> (\<not> (meets B A C D))"
hence "meets B A C D" by blast
		obtain T where "B \<noteq> A \<and> C \<noteq> D \<and> col B A T \<and> col C D T" using meet_f[OF `meets B A C D`]  by  blast
		...
		have "col A B T" using collinearorder[OF `col B A T`] by blast
		have "meets A B C D" using meet_b[OF `A \<noteq> B` `C \<noteq> D` `col A B T` `col C D T`] .
		show "False" using `meets A B C D` `A \<noteq> B \<and> C \<noteq> D \<and> \<not> (meets A B C D) \<and> same_side C D A B` by blast
	qed
	hence "\<not> (meets B A C D)" by blast
	have "B \<noteq> A" using inequalitysymmetric[OF `A \<noteq> B`] .
	have "same_side C D B A" using samesidesymmetric[OF `same_side C D A B`] by blast
	have "same_side D C B A" using samesidesymmetric[OF `same_side C D A B`] by blast
	...
	have "tarski_parallel B A C D" using tarski_parallel_b[OF `B \<noteq> A` `C \<noteq> D` `\<not> (meets B A C D)` `same_side C D B A`] .
	have "\<not> (meets B A D C)"
	proof (rule ccontr)
		assume "\<not> (\<not> (meets B A D C))"
hence "meets B A D C" by blast
		obtain T where "B \<noteq> A \<and> D \<noteq> C \<and> col B A T \<and> col D C T" using meet_f[OF `meets B A D C`]  by  blast
		have "C \<noteq> D" using `C \<noteq> D` .
	...
	have "tarski_parallel B A D C" using tarski_parallel_b[OF `B \<noteq> A` `D \<noteq> C` `\<not> (meets B A D C)` `same_side D C B A`] .
