...
proof -
	have "col B D C" using col_b `axioms` `bet B D C` by blast
	have "\<not> (A = D)"
	proof (rule ccontr)
		assume "A = D"
		have "col B A C" sorry
	...
	have "col A M D" using col_b `axioms` `bet A M D \<and> seg_eq M A M D` by blast
	...
	proof (rule ccontr)
		assume "C = M"
		have "col A C M" using col_b `axioms` `C = M` by blast
	...
	proof (rule ccontr)
		assume "B = M"
		have "col A B M" using col_b `axioms` `B = M` by blast
	...
	have "col B M F" using col_b `axioms` `bet B M F \<and> seg_eq M F M B` by blast
	have "M = M" using equalityreflexiveE[OF `axioms`] .
	have "col B M M" using col_b `axioms` `M = M` by blast
	...
	have "col A M A" using col_b `axioms` `A = A` by blast
	have "col A M D" using col_b `axioms` `bet A M D \<and> seg_eq M A M D` by blast
	...
	have "col E A F" using col_b `axioms` `bet E A F` by blast
