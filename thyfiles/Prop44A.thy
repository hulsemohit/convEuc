...
proof -
	...
	have "col A B E" using collinear_b `axioms` `bet A B E` by blast
	...
	have "col G B q" using collinear_b `axioms` `bet G B q \<and> seg_eq B q G B` by blast
	...
	have "col H A h" using collinear_b `axioms` `bet H A h \<and> ang_eq h A B A B G \<and> ang_eq h A B G B A \<and> ang_eq B A h G B A \<and> ang_eq H A B A B q \<and> ang_eq H A B q B A \<and> ang_eq B A H q B A \<and> parallel H h G q \<and> seg_eq H A B q \<and> seg_eq A h G B \<and> seg_eq A T T B \<and> seg_eq H T T q \<and> seg_eq G T T h \<and> bet H T q \<and> bet G T h \<and> bet A T B` by blast
	...
	have "same_side H G A B" using sameside_b[OF `axioms` `col A B T` `col A B B` `bet H T q` `bet G B q` `\<not> col A B H` `\<not> col A B G`] .
	...
	have "parallel G B E F \<and> parallel G F B E" using parallelogram_f[OF `axioms` `parallelogram G B E F`] .
	...
	have "col F E E" using collinear_b `axioms` `E = E` by blast
	have "col G B B" using collinear_b `axioms` `B = B` by blast
	have "col H A A" using collinear_b `axioms` `A = A` by blast
	...
	have "parallelogram G B E F" using `parallelogram G B E F` .
	have "parallel G F B E" using parallelogram_f[OF `axioms` `parallelogram G B E F`] by blast
	have "parallel H G A B" using parallelogram_f[OF `axioms` `parallelogram H A B G`] by blast
	...
	have "same_side G F B E" using tarski_parallel_f[OF `axioms` `tarski_parallel B E G F`] by blast
	have "same_side H G A B" using tarski_parallel_f[OF `axioms` `tarski_parallel A B H G`] by blast
	have "col A B E" using collinear_b `axioms` `bet A B E` by blast
	...
	have "bet A t F \<and> seg_eq A t t F" using midpoint_f[OF `axioms` `midpoint A t F`] .
	...
	have "col F E K" using collinear_b `axioms` `bet H B K \<and> bet F E K` by blast
	...
	have "parallel A H G B" using parallelogram_f[OF `axioms` `parallelogram A H G B`] by blast
	...
	have "parallel B E M K" using parallelogram_f[OF `axioms` `parallelogram B M K E`] by blast
	...
	have "col M K K" using collinear_b `axioms` `K = K` by blast
	have "col B E E" using collinear_b `axioms` `E = E` by blast
	have "col G F F" using collinear_b `axioms` `F = F` by blast
