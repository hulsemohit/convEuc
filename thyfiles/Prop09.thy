...
proof -
	...
	proof (rule ccontr)
		assume "B = E"
		have "col A B E" using col_b `axioms` `B = E` by blast
	...
	proof (rule ccontr)
		assume "col B A F"
		have "col B F E" using col_b `axioms` `bet B F E \<and> seg_eq F B F E` by blast
	...
	proof (rule ccontr)
		assume "A = F"
		have "col B A F" using col_b `axioms` `A = F` by blast
