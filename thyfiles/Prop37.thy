...
proof -
	...
	have "col B M A" using col_b `axioms` `bet E M C \<and> bet B M A` by blast
	have "col B m D" using col_b `axioms` `bet F m C \<and> bet B m D` by blast
