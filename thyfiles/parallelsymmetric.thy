...
theorem(in euclidean_geometry) parallelsymmetric:
	assumes 
		"parallel A B C D"
...
hence "meets C D A B" by blast
		obtain P where "C \<noteq> D \<and> A \<noteq> B \<and> col C D P \<and> col A B P" using meet_f[OF `meets C D A B`]  by  blast
	...
	have "parallel C D A B" using parallel_b[OF `C \<noteq> D` `A \<noteq> B` `col C D c` `col C D d` `c \<noteq> d` `col A B a` `col A B b` `a \<noteq> b` `\<not> (meets C D A B)` `bet c m b` `bet a m d`] .
