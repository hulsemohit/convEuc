...
proof -
	...
	next
		case 2
		have "col B C D" using col_b `axioms` `B = D` by blast
	next
		case 3
		have "col B C D" using col_b `axioms` `C = D` by blast
	next
		case 4
	...
	next
		...
		proof (cases)
			case 1
			have "col A B D" using `col A B D` .
			have "A = B \<or> A = D \<or> B = D \<or> bet B A D \<or> bet A B D \<or> bet A D B" using col_f[OF `axioms` `col A B D`] .
			...
			next
				...
				have "col D B C" using col_b `axioms` `bet D B C` by blast
				have "col B C D" using collinearorder[OF `axioms` `col D B C`] by blast
			next
				case 3
		...
		next
			...
			next
				...
				have "col B C D" using col_b `axioms` `bet C B D` by blast
			next
				case 3
				have "\<not> (\<not> col B C D)"
				proof (rule ccontr)
