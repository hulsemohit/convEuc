...
proof -
	...
	have "col B M C" using col_b `axioms` `bet B M C` by blast
	...
	have "col E L D" using col_b `axioms` `bet E L D` by blast
	...
	proof (rule ccontr)
		...
		proof (rule ccontr)
			...
			have "col E L L" using col_b `axioms` `L = L` by blast
			have "meets E L M C" sorry
			have "\<not> (meets E L M C)" sorry
		...
		proof (rule ccontr)
			...
			have "col M C C" using col_b `axioms` `C = C` by blast
			have "meets E L M C" sorry
			have "\<not> (meets E L M C)" sorry
			show "False" using `\<not> (meets E L M C)` `meets E L M C` by blast
		qed
		...
		have "L = L" using equalityreflexiveE[OF `axioms`] .
		have "col C L L" using col_b `axioms` `L = L` by blast
		have "bet D L E" using betweennesssymmetryE[OF `axioms` `bet E L D`] .
		have "bet M P E" using `bet M P E` .
		have "col C L P" using col_b `axioms` `bet M P E \<and> bet C P L` by blast
		have "same_side D M C L" sorry
		...
		have "C = C" using equalityreflexiveE[OF `axioms`] .
		have "col C C L" using col_b `axioms` `C = C` by blast
	...
	obtain R where "bet B R L \<and> bet D R C" sorry
	...
	have "ray_on C M B" using ray4 `axioms` `bet C M B` `C \<noteq> M` by blast
	have "C = C" using equalityreflexiveE[OF `axioms`] .
	have "col C C E" using col_b `axioms` `C = C` by blast
	...
	proof (rule ccontr)
		...
		proof (rule ccontr)
			assume "col B C E"
			have "E = E" using equalityreflexiveE[OF `axioms`] .
			have "col D E E" using col_b `axioms` `E = E` by blast
			have "meets B C D E" sorry
			have "\<not> (meets B C D E)" sorry
	...
	obtain T where "bet B T E \<and> bet D T C" sorry
