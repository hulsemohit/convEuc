...
hence "col A D B" by blast
	...
	have "triangle A D B" using triangle_b[OF `axioms` `\<not> col A D B`] .
	have "seg_eq A D A B" using `bet A D C \<and> seg_eq A D A B` by blast
	have "tri_isos A D B" using isosceles_b[OF `axioms` `triangle A D B` `seg_eq A D A B`] .
...
hence "col A B D" by blast
	...
	have "ang_lt A B D A B C" using anglelessthan_b[OF `axioms` `bet A D C` `ray_on B A A` `ray_on B C C` `ang_eq A B D A B D`] .
