...
theorem(in euclidean_geometry) samesideflip:
	assumes 
		"same_side P Q A B"
...
proof -
	...
	have "same_side P Q B A" using sameside_b[OF `col B A p` `col B A q` `bet P p r` `bet Q q r` `\<not> col B A P` `\<not> col B A Q`] .
