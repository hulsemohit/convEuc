...
hence "col B C A" by blast
		have "col C D B" using collinear_b `axioms` `bet C D B \<and> seg_eq D B C D` by blast
		...
		have "meets D C E F" using meet_b[OF `axioms` `D \<noteq> C` `E \<noteq> F` `col D C A` `col E F A`] .
		have "\<not> (meets D C E F)" using parallel_f[OF `axioms` `parallel D C E F`] by fastforce
	...
	obtain R where "bet b R D \<and> bet C R M" using Pasch_innerE[OF `axioms` `bet b M B` `bet C D B` `\<not> col b B C`]  by  blast
	...
	obtain Q where "bet b Q R \<and> bet C Q A" using Pasch_innerE[OF `axioms` `bet b A c` `bet C R c` `\<not> col b c C`]  by  blast
	...
	have "col c A b" using collinear_b `axioms` `bet c A b \<and> ang_eq b A D A D B \<and> ang_eq b A D B D A \<and> ang_eq D A b B D A \<and> ang_eq c A D A D C \<and> ang_eq c A D C D A \<and> ang_eq D A c C D A \<and> parallel c b B C \<and> seg_eq c A D C \<and> seg_eq A b B D \<and> seg_eq A M M D \<and> seg_eq c M M C \<and> seg_eq B M M b \<and> bet c M C \<and> bet B M b \<and> bet A M D` by blast
