...
proof -
	...
	next
		...
		have "seg_eq D B d b" using n5_lineE[OF `axioms` `seg_eq A B a b` `seg_eq C D c d` `seg_eq A D a d` `bet C A B` `bet c a b` `seg_eq C A c a`] .
	...
	next
		...
		have "seg_eq D B d b" using n5_lineE[OF `axioms` `seg_eq C B c b` `seg_eq A D a d` `seg_eq C D c d` `bet A C B` `bet a c b` `seg_eq A C a c`] .
		have "seg_eq B D b d" using congruenceflip[OF `axioms` `seg_eq D B d b`] by blast
		thus ?thesis by blast
	qed
	thus ?thesis by blast
