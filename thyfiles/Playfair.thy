...
hence "\<not> (cross A D B C \<or> cross A C B D)" by blast
		...
		obtain p where "bet A p C \<and> bet D p B" using cross_f[OF `axioms` `cross A C D B`]  by  blast
		...
		have "cross A C B D" using cross_b[OF `axioms` `bet A p C` `bet B p D`] .
	...
	next
		...
		obtain p where "bet A p C \<and> bet B p D" using cross_f[OF `axioms` `cross A C B D`]  by  blast
		...
		have "cross B D A C" using cross_b[OF `axioms` `bet B p D` `bet A p C`] .
