...
proof -
	...
	have "seg_eq A B B C \<and> seg_eq B C C A" sorry
	...
	proof (rule ccontr)
		assume "C = B"
		have "col A C B" using col_b `axioms` `C = B` by blast
	...
	proof (rule ccontr)
		assume "C = A"
		have "col B C A" using col_b `axioms` `C = A` by blast
	...
	proof (rule ccontr)
		assume "col D C E"
		have "col C A E" using col_b `axioms` `bet C A E \<and> seg_eq A E A B` by blast
		have "col C B D" using col_b `axioms` `bet C B D \<and> seg_eq B D A B` by blast
	...
	have "cuts A D E B G" sorry
	have "bet A F D" using `bet A F D` .
	have "bet E F B" using `bet E F B` .
	have "cuts A D E B F" sorry
	have "\<not> (col D E B)"
	proof (rule ccontr)
		assume "col D E B"
		have "col C B D" using col_b `axioms` `bet C B D \<and> seg_eq B D A B` by blast
	...
	have "seg_eq M F M F" using congruencereflexiveE[OF `axioms`] .
