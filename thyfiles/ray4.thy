...
theorem(in euclidean_geometry) ray4:
	assumes 
...
proof -
	...
	have "bet J A B" using betweennesssymmetryE[OF `bet B A J`] .
	consider "bet A E B"|"E = B"|"bet A B E" using `bet A E B \<or> E = B \<or> bet A B E`  by blast
	hence "ray_on A B E"
	proof (cases)
		assume "bet A E B"
		have "bet J A B" using `bet J A B` .
		have "bet J A E" using innertransitivityE[OF `bet J A B` `bet A E B`] .
		have "ray_on A B E" using ray_b[OF `bet J A E` `bet J A B`] .
	...
	next
		assume "bet A B E"
		have "bet J A B" using `bet J A B` .
		have "bet J A E" using n3_7b[OF `bet J A B` `bet A B E`] .
		have "ray_on A B E" using ray_b[OF `bet J A E` `bet J A B`] .
		thus ?thesis by blast
	qed
	thus ?thesis by blast
