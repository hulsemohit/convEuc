...
proof -
	...
	have "parallel A G B C" using parallelogram_f[OF `axioms` `parallelogram A G B C`] by blast
	...
	have "parallel D H F E" using parallelogram_f[OF `axioms` `parallelogram D H F E`] by blast
	...
	have "oppo_side G B A C" using oppositeside_b[OF `axioms` `bet G m C` `col B A m` `\<not> col B A G`] .
	have "oppo_side C B A G" using oppositesidesymmetric[OF `axioms` `oppo_side G B A C`] .
	have "oppo_side H F D E" using oppositeside_b[OF `axioms` `bet H M E` `col F D M` `\<not> col F D H`] .
