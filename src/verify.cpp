...
namespace verify {
    ...
    string test_combination(const string& stmt, const theorem& th,
                            const string& vars, const string& sub,
                            const fact_set& facts) {
        if (sub.size() == th.get_var_cnt()) {
            theorem th_ = th.instantiate(sub);
            if (!facts.verify_facts(th_.assumptions)) return "";

            fact_set f;
            f.add_fact(th_.conclusion);
            if (f.trace_logical(stmt) != "FALSE") return sub;
            ...
            string s;
            for (char c: vars + "?") {
                if (th.is_exists_pos(sub.size()) && facts.has_var(c)) continue;
                if (c == '?' && !th.is_exists_pos(sub.size())) continue;
                ...
                if (!facts.verify_facts(th_.assumptions)) continue;

                fact_set tmp;
                tmp.add_fact(th_.conclusion);
                if (tmp.trace_logical(stmt, true) == "FALSE") continue;

                if (std::all_of(th_.assumptions.begin(), th_.assumptions.end(),
                                [](string str) {
                                    return str.find("?") == string::npos;
                                })) {
                    if (tmp.trace_logical(stmt) != "FALSE") return sub_;
                }

                int missing_count{};
                for (char x: parse::get_vars(stmt))
                    if ((sub + c).find(x) == string::npos) missing_count++;
    ...
    string test_combination_slow(const string& stmt, const theorem& th,
        ...
        fact_set tmp;
        tmp.add_fact(th_.conclusion);
        if (tmp.trace_logical(stmt, true) == "FALSE") return "";

        if (std::all_of(
                th_.assumptions.begin(), th_.assumptions.end(),
                [](string str) { return str.find("?") == string::npos; })) {
            return sub;
        }
        ...
        for (char c: vars)
            if (good_vars.find(c) == string::npos) {
                sub[i]   = c;
                string s = test_combination_slow(stmt, th, vars, sub, facts);
                if (!s.empty()) return s;
            }
    ...
    // and facts.
    string generate_reason(string name, const theorem& th, const string& stmt,
                           const fact_set& facts) {
        name = utils::fix_name(name);
        bool b{true};
        for (string h: th.assumptions)
            if (!facts.has_fact(h)) b = false;

        if (b) {
            string res = "using " + name;
            ...
            for (string h: th.assumptions) {
                if (string s = facts.trace_logical(h); s != "FALSE") {
                    ...
                    Warn("Attempted to use theorem " + name + " to prove "
                         + stmt
                         + " with incomplete preconditions, using sorry.");
                    return "sorry";
                }
            }
            res += " by blast";
    ...
    string by_equalitysub(const string& stmt, const fact_set& facts) {
        vector<string> possibilities{facts.all_matches(
            theorem({}, stmt)
                .instantiate(string(parse::get_vars(stmt).size(), '?'))
                .conclusion)};
        ...
        for (string s: possibilities) {
            ...
            for (int i{}; i < stmt.size(); ++i)
                if (stmt[i] != s[i]) {
                    if (string k
                        = facts.trace_logical(string("EQ") + s[i] + stmt[i]);
                        k != "FALSE") {
                        reason += "`" + parse::translate(k) + "` ";
                    } else if (k = facts.trace_logical(string("EQ") + stmt[i]
                                                       + s[i]);
                               k != "FALSE") {
                        reason += "`" + parse::translate(k) + "` ";
                    } else {
            ...
            if (reason != "FALSE") return reason + "by blast";
        }
    ...
    string by_axiom(const string& stmt, const string& ax, const string& vars,
                    const fact_set& facts) {
        if (ax == "equalitysub") return by_equalitysub(stmt, facts);
        ...
        Warn("Failed to use axiom `" + ax + "` to show `" + stmt
             + "`, using sorry.");
        return "sorry";
    }

    // Proof using a theorem.
    string by_theorem(const string& stmt, const string& th, const string& vars,
                      const fact_set& facts) {
        ...
        Warn("Failed to use theorem `" + th + "` to show `" + stmt
             + "`, using sorry.");
        return "sorry";
    }
    ...
    string by_definition(const string& stmt, string def,
        ...
        Warn("Failed to use definition `" + def + "` to show `" + stmt
             + "`, using sorry.");
        return "sorry";
    }

    // Separates the type of reason from the reason (eg. cn, axiom, lemma
    // etc...)
    ...
    string verify(const string& vars, const fact_set& facts, const string& stmt,
                  const string& reason) {
        ...
        if (type == "")
            ...
            return by_theorem(stmt,
                              utils::fix_name(utils::split_at(reason, ":")[1]),
                              vars, facts);
            ...
            return by_definition(stmt, utils::split_at(reason, ":")[1], vars,
                                 facts);

        else if (type == "assumption" || type == "reductio")
            Abort("Arguments by contr should be handled in a different "
                  "function.");

        else if (type == "cases")
            Abort("Arguments by cases should be handled in a different "
                  "function.");
