...
bool matches(const string& fact, const string& pattern) {
    if (fact.size() != pattern.size()) return false;
    for (int i{}; i < fact.size(); ++i)
        if (pattern[i] != '?' && fact[i] != pattern[i]) return false;
    return true;
}
...
string fact_set::trace_logical(string fact, bool reverse) const {
    ...
    for (string t: facts) {
        ...
        if (reverse ? matches(fact, s) : matches(s, fact)) return t;
        if (s.substr(0, 2) == "AN") {
            vector<string> v = utils::split_at(s.substr(2), "+");
            for (string f: v)
                if (reverse ? matches(fact, f) : matches(f, fact)) { return t; }
        }
    }
    ...
    if (fact.substr(0, 2) == "OR") {
        ...
        for (string c: v)
            if (string s = trace_logical(c); s != "FALSE") return s;
...
bool fact_set::verify_facts(const vector<string>& fact) const {
    for (const string& f: fact)
        if (trace_logical(f) == "FALSE") return false;
    return true;
}
...
// A special check for disjunctions: if another disjunction which
...
vector<string> fact_set::all_matches(const std::string& pattern) const {
    vector<string> res;
    for (string s: facts)
        if (matches(s, pattern)) res.push_back(s);
    return res;
}
