...
void generate_base_theory() {
    ...
    for(auto [name_, th]: def_list) {
        if(name_.back() != 'f')
            continue;
        string name = name_; name.pop_back(); name.pop_back();
    ...
    for(auto [name_, th]: axioms) {
        ...
        for(string s: th.assumptions)
            vars += parse::get_vars(s);
        utils::unique(vars);

        string exists = th.get_exists();
        if(name_ == "equalityreflexive"
        || name_ == "congruencereflexive"
        || name_ == "equalityreverse"
        || name_ == "betweennessidentity"
        || name_ == "nullsegment2") {
            vars += exists; exists = "";
