    ...
    if (cmd == "NO")
        ...
        for (string s: clauses) {
            for (auto [v, p]: get_pos(s))
                for (int k: p) res[v].push_back(k + offset);
            offset += s.size() + 1;
        }
...
theorem::theorem(vector<string> euc_assms, string euc_conc)
    ...
    for (string s: euc_assms) {
        ...
        for (auto c: assms.back()) vars += c.first;
    }
    ...
    if (vars.size() == exists.size()) exists = "";

    // circle_f
    if (conclusion == "CIXCAB") exists = "X";
    // deZolt1
    if (conclusion == "NOETDBCEBC") exists = "";
    ...
    for (std::map<char, vector<int>> m: assms) {
        ...
        for (int i{}; i < var_cnt; ++i) assms_pos.back().push_back(m[vars[i]]);
    }

    for (int i{}; i < var_cnt; ++i) conc_pos.push_back(conc[vars[i]]);
...
theorem theorem::instantiate(const string& vars) const {
    ...
    for (int i{}; i < var_cnt; ++i) {
        for (int j{}; j < assumptions.size(); ++j)
            for (int k: assms_pos[j][i]) assms_copy[j][k] = vars[i];

        for (int k: conc_pos[i]) conc_copy[k] = vars[i];
    }
...
string theorem::to_string() const {
    string s{};
    for (string a: assumptions) s += a + " ";
    s += "==> " + conclusion;
    return s;
}

int theorem::get_var_cnt() const { return var_cnt; }

string theorem::get_exists() const { return exists; }

void theorem::add_depends(const string& s) { depends.insert(s); }

std::set<string> theorem::get_depends() const { return depends; }

bool theorem::is_exists_pos(int p) const {
    return std::find(exists_pos.begin(), exists_pos.end(), p)
           != exists_pos.end();
}
